...
char login_name[20];           /* store user's login for sys.cpp & rcs.cpp    */
...
static HocParmLimits _hoc_parm_limits[] = {
    ...
    (char*)"diam", 1e-9, 1e9,
    (char*)"cm", 0., 1e9,
    (char*)"rallbranch", 1., 1e9,
    (char*)"nseg", 1., 1e9,
    (char*)"celsius", -273., 1e6,
    (char*)"dt", 1e-9, 1e15,
	0, 0., 0.
};
...
static HocParmUnits _hoc_parm_units[] = {
    ...
    (char*)"diam",     (char*)"um",
    (char*)"cm",       (char*)"uF/cm2",
    (char*)"celsius",  (char*)"degC",
    (char*)"dt",       (char*)"ms",
    (char*)"t",        (char*)"ms",
    (char*)"v",        (char*)"mV",
    (char*)"i_cap",    (char*)"mA/cm2",
	0, 0
};
...
void hoc_last_init(void)
{
	...
	if (!nrn_mech_dll && !nrn_noauto_dlopen_nrnmech) {
		...
		if (ff) {
			fclose(ff);
			nrn_mech_dll = (char*)DLL_DEFAULT_FNAME;
		}
	}
	if (nrn_mech_dll) {
		char *cp1, *cp2;

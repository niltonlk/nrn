...
Section* nrnpy_newsection(NPySecObj* v) {
	...
	sec->prop->dparam[PROP_PY_INDEX]._pvoid = (void*)v;
...
void hoc_secname(void) {
	...
	Section* sec = chk_access();
	if (!buf) { buf = static_cast<char *>(emalloc(256*sizeof(char))); }
...

{
    ...
    if (!global && hoc_thisobject) {
    	sym = hoc_table_lookup(str, hoc_thisobject->ctemplate->symtable);
    }else{
	hoc_objectdata = hoc_top_level_data;
	sym = hoc_lookup(str);
    }

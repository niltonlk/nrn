...
bool boolean_dialog(const char* label, const char* accept,
  const char* cancel, Window* w, Coord x, Coord y) {
	...
	vbox->append(
		l.hcenter(
		   l.hbox(
			k.push_button(accept, new DialogAction(d, true)),
			l.hspace(10),
...
void continue_dialog(const char* label, Window* w, Coord x, Coord y) {
	...
	vbox->append(
		l.hcenter(
			k.push_button("Continue", new DialogAction(d, true))
		)
	);
	if (w) {
		d->post_for(w);
...
FieldDialog::FieldDialog(Glyph* g, Style* s): Dialog(g, s) {
}
...
FieldDialog* FieldDialog::field_dialog_instance(const char* str, Style* style,
	Glyph* extra)
{
	Glyph* g;
    ...
    g = 
        widgets.inset_frame(
            layout.margin(
		layout.flexible(vb, fil, 0),
                10.0
            )
        );

	FieldDialog* fd = new FieldDialog(g, style);
	FieldEditor* fe = dialogs.field_editor(str, style,
		new FieldEditorCallback(FieldDialog)(
			fd, &FieldDialog::accept, &FieldDialog::cancel
		)
	);
  ...
  const char* cancel) {
	...
	vbox->append(
	   l.hcenter(
	        l.hflexible(
		   l.hbox(
			k.push_button(accept, new DialogAction(d_, true)),
			l.hspace(10),
...
OcGlyphDialog::OcGlyphDialog(Glyph* g, Style* s) : Dialog(g, s) {}

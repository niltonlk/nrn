...
//SymDirectory
SymDirectory::SymDirectory(const String& parent_path, Object* parent_obj,
...
//		obd = hoc_objectdata;
	...
	impl_->make_pathname(parent_path.string(), sym->name,
		hoc_araystr(sym, array_index, obd), suffix);
	switch(sym->type) {
#if CABLE
	case SECTION:
	...
	case OBJECTALIAS:
		...
		break;
	default:
		hoc_execerror("Don't know how to make a directory out of",
			path().string());
		break;
	}
	impl_->sort();
...
void SymDirectoryImpl::make_pathname(const char* parent, const char* name,
  const char* index, int suffix) {
  	char buf[200];

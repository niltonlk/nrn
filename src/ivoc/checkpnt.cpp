...
public:
	...
	void size(unsigned long s) { size_ = s;}
...
#undef Chk
#define Chk(arg1,arg2) if (!(arg1)) { printf("%s line %d\n", arg2, lineno_); return false;}
#undef Get
#define Get(arg) if (!get(arg)) { return false; }
...
bool OcReadChkPnt::symbol() {
	...
	Get(i);	sym->cpublic = i;
	Get(i); sym->s_varn = i;
	Get(i); sym->defined_on_the_fly = i;
	switch (type) {
	case VAR:
	   {
		if (subtype == NOTUSER) {

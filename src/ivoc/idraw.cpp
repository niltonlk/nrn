...
static void rgbcolor(const Color* c,
...
void OcIdraw::text(Canvas*, const char* s, const Transformer& t,
...
// 3/12/95
	...
	sprintf(buf, "%%I cfg %x%x%x\n%f %f %f SetCFg\n",
	  int(r*256),int(g*256),int(b*256), r, g, b);
	out << "Begin %I Text\n";
	out << buf;
	if (font) {
...
void OcIdraw::mline(Canvas*, int count, const Coord* x, const Coord* y,
   const Color* color, const Brush* b) {
...
void OcIdraw::bspl(Canvas*, int count, const Coord* x, const Coord* y,
   const Color* color, const Brush* b) {
	char buf[100];
	out << "\nBegin %I BSpl\n";
...
void OcIdraw::line(Canvas*, Coord x1, Coord y1, Coord x2, Coord y2,
   const Color* color, const Brush* b) {
#if 1
	out << "\nBegin %I Line\n";
	brush(b);
...
void OcIdraw::close_path() {
	closed_ = true;
	if (curved_) {
		curve_to(xpath_[0], ypath_[0], xpath_[ipath_-1], ypath_[ipath_-1], xpath_[0], ypath_[0]);
	}
}

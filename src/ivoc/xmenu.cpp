...
public:
	...
	static bool has_old_focus() { bool old = sema_; sema_ = false; return old; }
...
ValEdLabel::ValEdLabel(Glyph* g) : MonoGlyph(g) {
	state_ = false;
	if (!color_) {
		color_ = Color::lookup(Session::instance()->default_display(),
			"yellow");
		Resource::ref(color_);
	}
	tts_ = NULL;
...
public:
	...
	OcTelltaleGroup* group() { return g_; }
	void start();
	void stop();
private:
	OcTelltaleGroup* g_;
...
void hoc_ivpvaluerun(const char* name, double* pd, const char* action,
	bool deflt, bool canRun, HocSymExtension* extra){
	checkOpenPanel();
	hoc_radio->stop();
...
void hoc_ivslider(double* pd, float low, float high,
	float resolution, int nsteps, const char* s, bool vert,
	bool slow, Object* pyvar, Object* pyact) {
	checkOpenPanel();
...
void HocPanel::save_all(ostream&) {
	...
	if (hoc_panel_list) for (i = 0; i < cnt; ++i) {
...
static void var_freed(void* pd, int size) {
	if (hoc_panel_list) for (long i = hoc_panel_list->count() - 1; i >= 0; --i) {
...
//HocUpdateItem
HocUpdateItem::HocUpdateItem(const char* name, HocItem* hi) : HocItem(name, hi) {}
HocUpdateItem::~HocUpdateItem() {
	HocPanel::keep_updated(this, false);
}
void HocUpdateItem::update_hoc_item() {}
void HocUpdateItem::check_pointer(void*, int) {}void HocUpdateItem::data_path(HocDataPaths*, bool) {}
...
HocPushButton::HocPushButton(const char* name, HocAction* a, HocItem* hi) : HocItem(name, hi) {
	a_ = a;
	Resource::ref(a);
	a->hoc_item(this);
}
...
HocRadioButton::HocRadioButton(const char* name, HocRadioAction* a, HocItem* hi) : HocItem(name, hi) {
	a_ = a;
	Resource::ref(a);
	a->hoc_item(this);
}
...
void HocPanel::label(const char* name) {
	box()->append(
	    LayoutKit::instance()->margin(
		   WidgetKit::instance()->label(name), 3
            )				       
        );
...
void HocPanel::slider(double* pd, float low, float high,
  ...
  const char* send, bool vert, bool slow, Object* pyvar, Object* pysend) {
	...
	box()->append(
	    lk->hflexible(
		WidgetKit::instance()->vscroll_bar(s->adjustable())
	  )
	);
      } else {
	box()->append(
	    lk->hflexible(
		WidgetKit::instance()->hscroll_bar(s->adjustable())
	  )
	);
      }	
	if (slow) {
...
HocMenu::HocMenu(const char* name, Menu* m, MenuItem* mi, HocItem* hi, bool add2menubar) : HocItem(name, hi) {
	menu_ = m;
	mi_ = mi;
...
void HocPanel::valueEd(const char* name, const char* variable, const char* action, bool canrun,
	double* pd, bool deflt, bool keep_updated, HocSymExtension* extra,
...
//HocItem
HocItem::HocItem(const char* str, HocItem* hi) : str_(str){
	help_parent_ = hi;
}
HocItem::~HocItem() {
//	printf("~HocItem %s\n", str_.string());
...
//printf("HocVarLabel::update() freed\n");
		p_->body(
			LayoutKit::instance()->margin(
				WidgetKit::instance()->label("Free'd"), 3
			)
		);
		p_->redraw();
		p_->reallocate();
...
public:
	HocDefaultCheckbox(HocDefaultValEditor*, Glyph*, Style*, TelltaleState*,
	   Action*);
...
HocDefaultCheckbox::HocDefaultCheckbox(HocDefaultValEditor* dve, Glyph* g,
 Style* s, TelltaleState* t, Action* a) : Button(g, s, t, a)
{
	dve_ = dve;
}
...
HocDefaultValEditor::HocDefaultValEditor(const char* name, const char* variable,
  ValEdLabel* prompt, HocValAction* a, double* pd, bool canrun,
  HocItem* hi, Object* pyvar)
    : HocValEditor(name, variable, prompt, a, pd, canrun, hi, pyvar)
{
	checkbox_ = HocDefaultCheckbox::instance(this);
	checkbox_->ref();
	checkbox_->action(new ActionCallback(HocDefaultValEditor)
		(this, &HocDefaultValEditor::def_action));
	evalField();
...
static void set_format() {
	...
	if (!xvalue_format) {
		...
		WidgetKit::instance()->style()->find_attribute("xvalue_format",
			*xvalue_format);
		char buf[100];
...
HocValEditor::HocValEditor(const char* name, const char* variable, ValEdLabel* prompt,
  HocValAction* a, double* pd, bool canrun, HocItem* hi, Object* pyvar)
...
void HocValEditor::check_pointer(void* v, int size) {
	if (pval_) {
		...
		if (size == 1) {
			...
			if (pval_ < pd || pval_ >= pd + size) return;
		}
		pval_ = 0;
	}
}
...
public:
	StepperMenu();
	virtual ~StepperMenu();
	void stepper(HocValStepper* vs) { vs_ = vs;}
	HocValStepper* stepper() { return vs_; }
...
NrnUpDown::NrnUpDown(const Color* c) : Glyph() {
	color_ = c;
	Resource::ref(c);
}
...
HocValStepper::HocValStepper(HocValEditor* ve, Glyph* g, Style* s,
TelltaleState* t) : Stepper(g, s, t) {
	if (!menu_) {
...
double OcSlider::slider_val() {
	...
	x = x*resolution_;
	if (x > bv_->upper(Dimension_X) - resolution_/2.) x = bv_->upper(Dimension_X);
	if (x < bv_->lower(Dimension_X) + resolution_/2.) x = bv_->lower(Dimension_X);
...
//	b_->ref();	// mutually reffed so don't
	b_->action(new ActionCallback(HocStateButton)
		(this, &HocStateButton::button_action));
}
   ...
   : HocUpdateItem("", hi)
{
	...
	b_ = mi;
//	Resource::ref(b_);
	b_->action(new ActionCallback(HocStateMenuItem)
		(this, &HocStateMenuItem::button_action));
}

 ...
 * show_errmess_always()
 * errno not defined from include file in hoc.cpp
 ...
 * a number of things fixed while looking at the test coverage of hoc.cpp
 ...
 * support for ivoc/SRC/checkpoint.cpp
 ...
 * set term=vt125 replaces vt100 in plot.cpp since that is a common term
 ...
 * and use stack macros in code.cpp
 * 
 ...
 * don't inlclude stdio.h twice in ytab.cpp
 * 
 ...
 * error in x.cpp message when color doesn't exist
 * xlabel("string")
 * 
 ...
 * moved from ivoc/main.cpp to here so oc can use it too.
 ...
 * list.cpp hoclist.h moved from nrnoc to oc
 ...
 * botched last checkin for symbol.cpp. notifies interviews when hoc_free_val..
 * called
 * 
 * Revision 1.107  92/10/24  10:16:35  hines
 * noiv.cpp is place to put function stubs that only make sense with interviews
 ...
 * that can be printed. This list is turned off in hoc_oop.cpp
 * 
 ...
 * avoid saber warning with hoc_xpop not declared double in nocable.cpp
 * 
 ...
 * failed to checkin code2.cpp when chkarg got moved there
 * 
 ...
 * chkarg moved to code2.cpp so always available
 * 
 ...
 * hoc.cpp inadvertently checked in last time with sigsegvec removed
 * 
 ...
 * leave out an xflush in x.cpp so doesn't hang. error in cable part of code.cpp
 ...
 * STKDEBUG turned on in code.cpp in which every element also carries a type.
 * 
...
/* hoc */
char *RCS_hoc_version = (char*)"$Revision: 3 $";
char *RCS_hoc_date = (char*)"$Date: 2003-02-11 19:36:05 +0100 (Tue, 11 Feb 2003) $";

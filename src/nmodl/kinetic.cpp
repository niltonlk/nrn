...
diag(s->name,  (char*)"must be a STATE, CONSTANT, ASSIGNED, STEPPED, or INDEPENDENT");
	}
	if (q2) {
	...
	if (q1 != lastok) {
		if(!(s->subtype & ARRAY)) {
			diag((char*)"REACTION: MUST be scalar or array", (char *)0);
		}
...
void massagereaction(Item* qREACTION, Item* qREACT1, Item* qlpar, Item* qcomma, Item* qrpar)
{
	...
	/*SUPPRESS 440*/
	replacstr(qREACTION,  (char*)"/* ~");
	/*SUPPRESS 440*/
	Insertstr(qrpar,  (char*)")*/\n");
	/*SUPPRESS 440*/
	replacstr(qrpar,  (char*)"/*REACTION*/\n");
	rterm = (Rterm *)0;
}
...
void flux(Item* qREACTION, Item* qdir, Item* qlast)
{
	...
	if (SYM(qdir)->name[0] == '-') {
		...
		/*SUPPRESS 440*/
		replacstr(qlast,  (char*)"/*REACTION*/\n");
	}else{
		if (rterm->rnext || (rterm->num != 1)) {
			diag((char*)"flux equations involve only one state", (char *)0);
		}
...
ITERATE(q1, ldifuslist) {
	...
	if (sr == rterm->sym) {
		...
		if (*STR(q1->next) == '\0') {
			...
			diag(sr->name,  (char*)"gets a flux from more than one current");
		}
	}
	q1 = q1->next;
}
...
#endif
		/*SUPPRESS 440*/
		replacstr(qlast,  (char*)"/*FLUX*/\n");
	}
	/*SUPPRESS 440*/
	Insertstr(qlast,  (char*)")*/\n");
	/*SUPPRESS 440*/
	rterm = (Rterm *)0;
}
...
void massagekinetic(Item* q1, Item* q2, Item* q3, Item* q4, int sensused) /*KINETIC NAME stmtlist '}'*/
{
	...
	if ((fun->subtype & (DERF|KINF)) && fun->u.i) {
		diag((char*)"Merging kinetic blocks not implemented", (char *)0);
	}
	fun->subtype |= KINF;
	numlist++;
	fun->u.i = numlist;
	...
	replacstr(q1,  (char*)"\nstatic int");
	qv = insertstr(q3,  (char*)"()\n");
#if VECTORIZE
if (vectorize) {
	kin_vect1(q1, q2, q4);
vectorize_substitute(qv,  (char*)"(void* _so, double* _rhs, double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt)\n");
}
#endif
	qv = insertstr(q3,  (char*)"{_reset=0;\n");
#if VECTORIZE
Sprintf(buf, "{int _reset=0;\n");
...
#endif
	qv = insertstr(afterbrace,  (char*)"double b_flux, f_flux, _term; int _i;\n");
#if 0 && VECTORIZE
	vectorize_substitute(qv,  (char*)"int _i;\n");
#endif
	...
	*/
	...
	for (r1 = conslist; r1; r1 = r1->reactnext) {
		...
		if (!rt) {
		   diag((char*)"Failed to diagonalize the Kinetic matrix", (char *)0);
		}
	}
	...
	if (count == 0) {
		diag((char*)"KINETIC contains no reactions", (char *)0);
	}
	fun->used = count;
	...
	Linsertstr(procfunc, buf);
	insertstr(q4,  (char*)"  } return _reset;\n");
...
for (j=0; j<2; j++) {
	for (rt = r->rterm[j]; rt; rt = rt->rnext) {
		...
		Insertstr(q,  (char*)"(f_flux - b_flux);\n");
	}
}
...
void genfluxterm(Reaction* r, int type, int n)
{
	...
	rt = r->rterm[0];
	if (!(rt->isstate)) {
		diag(rt->sym->name,  (char*)"must be (solved) STATE in flux reaction");
	}
	Sprintf(buf, "D%s", rt->sym->name);
	s = lookup(buf);
	if (rt->sym->varnum < ncons)
		diag(rt->sym->name,  (char*)"is conserved and has a flux");
	/* the right hand side */
	Insertstr(q,  (char*)"f_flux = b_flux = 0.;\n");
	if (type) {
		Sprintf(buf, "_RHS%d(", n);
		Insertstr(q, buf);
		if (rt->str) {
...
void kinetic_implicit(Symbol* fun, char* dt, char* mname)
{
	...
	if (rlst->sens_parm) {
		diag((char*)" SENS unimplemented for default kinetic integration",
			 (char*)"method");
	}
...
#endif
    CVODE_FLAG {
		Insertstr(rlst->position,  (char*)"  b_flux = f_flux = 0.;\n");
    }
...
for(_i=%d;_i<%d;_i++){\n",
	...
	qv = insertstr(rlst->position,  (char*)"");
	...
	qv = insertstr(rlst->position,  (char*)"");
#if 0 && VECTORIZE
	vectorize_substitute(qv,  (char*)"    } /* ILOOPEND */\n");
#endif
	Sprintf(buf,"    \n}");
	Insertstr(rlst->position, buf);
...
/* the list of states is in rterm at this time with the first at the end */
	...
	/*SUPPRESS 440*/
	replacstr(q1,  (char*)"/*");
	qv = insertstr(q1,  (char*)"");
#if 0 && VECTORIZE
	vectorize_substitute(qv, instance_loop());
#endif
	/*SUPPRESS 440*/
	Insertstr(q5->next,  (char*)"*/\n");
	/*SUPPRESS 440*/
	conslist->position = insertstr(q5->next->next,  (char*)"/*CONSERVATION*/\n");
#if 0 && VECTORIZE
	vectorize_substitute(conslist->position,  (char*)"/*CONSERVATION*/\n\
} /*ILOOPEND*/\n");
#endif
	rterm = (Rterm *)0;
}
...
int genconservterms(int eqnum, Reaction* r, int fn, Rlist* rlst)
{
	...
	for (rt = r->rterm[0]; rt; rt = rt->rnext) {
		...
		if (!(rt->isstate)) {
diag(rt->sym->name,  (char*)": only (solved) STATE are allowed in CONSERVE equations.");
		}
		if (rt->str) {
			if(rlst->capacity[rt->sym->used][0]) {
				Sprintf(buf, "_i = %s;\n", rt->str);
...
/* also index of symorder goes into sym->used */
	...
	for (rlst = rlist; rlst && (rlst->sym != fun); rlst = rlst->rlistnext)
		clst = clst->rlistnext;
	if (rlst == (Rlist *)0) {
		diag(fun->name,  (char*)"doesn't exist");
	}
	*prlst = rlst;
	*pclst = clst;
	/* Number the states. */
	...
	if (fun->u.i >= MAXKINBLK) {
		diag((char*)"too many solve blocks", (char*)0);
	}
...
{
	if (standard != actual) {
		diag(mes,  (char*)"not allowed in this kind of block");
	}
}
...
compart: COMPARTMENT NAME ',' expr '{' namelist '}'
	...
	| COMPARTMENT error {myerr( (char*)"Correct syntax is:	\
...
void massagecompart(Item* qexp, Item* qb1, Item* qb2, Symbol* indx)
{
	...
	if (indx) {
		...
		/*SUPPRESS 440*/
		Insertstr(qexp->prev->prev->prev,  (char*)"/*");
	}else{
		/*SUPPRESS 440*/
		Insertstr(qexp->prev,  (char*)"/*");
	}
	for (q = qb1->next; q != qb2; q = qs) {
		...
		if (!(SYM(q)->subtype & STAT) && in_solvefor(SYM(q))) {
			dlete(q);
#if 0
diag(SYM(q)->name,  (char*)"must be a (solved) STATE in a COMPARTMENT statement");
#endif
		}
	}
	if (!compartlist) {
...
void massageldifus(Item* qexp, Item* qb1, Item* qb2, Symbol* indx)
{
	...
	for (q = qb1->next; q != qb2; q = qs) {
		...
		if (!(s->subtype & STAT) && in_solvefor(s)) {
			dlete(q);
diag(SYM(q)->name,  (char*)"must be a (solved) STATE in a LONGITUDINAL_DIFFUSION statement");
		}
		...
		if (s != s2) {
			diag(SYM(q)->name,  (char*)"must be declared in COMPARTMENT");
		}
		lappendstr(ldifuslist,  (char*)"0"); /* will be flux conc index if any */
		lappendstr(ldifuslist,  (char*)""); /* will be dflux/dconc if any */
	}
}
...
void kin_vect3(Item* q1, Item* q2, Item* q4)
{
	...
	fun = SYM(q2);
	last = insertstr(q4->next,  (char*)"\n");
	first = insertstr(last,  (char*)"\n/*copy of previous function */\n");
...
void see_ostmt() {
	...
	if (vectorize) {
		if (astmt_state) {
			astmt_state = 0;
			vectorize_substitute(astmt_last,  (char*)"\n } /*ILOOPEND*/\n");
		}
	}	
#endif
}
...
void see_astmt(Item* q1, Item* q2)
{
	...
	if (vectorize) {
		...
		astmt_last = insertstr(q2->next,  (char*)"");
	}
#endif
}
...
void cvode_kinetic(Item* qsol, Symbol* fun, int numeqn, int listnum)
{
	...
	*/
	...
	kinetic_intmethod(fun,  (char*)"NEURON's CVode");
	...
	vectorize_substitute(procfunc->prev, buf);
	cvode_flag = 1;
	kinetic_implicit(fun,  (char*)"dt",  (char*)"ZZZ");
	cvode_flag = 0;
	pbeg = procfunc->prev;
	copyitems(cvode_sbegin, cvode_send, procfunc->prev);
	pend = procfunc->prev;

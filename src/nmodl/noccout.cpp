...
P("#include \"md2redef.h\"\n");
   ...
   if (currents->next != currents) {
	...
	if (state_discon_list_) {
	  ...
	  printlist(end_dion_stmt((char*)".001"));
	  P(" _g = (_g - _rhs)/.001;\n");
...
diag((char*)"current can only be LOCAL in a BREAKPOINT if CONDUCTANCE statements are used. ", SYM(q)->name);
		}
		Sprintf(buf, " _current += %s;\n", SYM(q)->name);
		P(buf);
	}
...
static void conductance_cout() {
  ...
  ITERATE(q, modelfunc) {
    if (q->itemtype == SYMBOL) {
      ...
      diag((char*)"modelfunc contains item which is not a SYMBOL or STRING", (char*)0);
    }
  }
  /* eliminate first { */
  ITERATE(q, m) {

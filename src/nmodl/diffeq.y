...
arglist: /*nothing*/ {
		$$ = list4((char*)"", (char*)"0.0", (char*)"0.0", (char*)"");
		}
	| arg { $$ = $1; }
	| arglist arg {
		$$ = $2;
...
static int yylex() {
	...
	case SYMBOL:
		...
		if (s == state) {
			fullname(buf);
			yylval.list = list4(buf, (char*)"1.0", (char*)"1.0", (char*)"0.0");
			rval = ATOM;
		}else{
			switch (s->name[0]) {
			case '+':
			...
			default:
				fullname(buf);
				yylval.list = list4(buf,  (char*)"0.0",  (char*)"0.0", buf);
				rval = ATOM;
				break;
			}
		}
	...
	default: /* had better be a STRING */
		...
		yylval.list = list4(STR(qexpr),  (char*)"0.0",  (char*)"0.0", STR(qexpr));
		rval = ATOM;
		break;
	}

	...
	*/
	...
	if (!level) { /* No multiple declarations at the root level and
		...
		if (sym->usage & EXPLICIT_DECL) {
			diag((char*)"Multiple declaration of ", sym->name);
		}
	...
	if (level < sym->level) { /* new one takes precedence */
		...
		diag(sym->name,  (char*)"has different values at same level");
	}
}
...
Symbol *ifnew_parminstall(char* name, char* num, char* units, char* limits)
{
	...
	if (!(s->subtype & (PARM | STEP1))) {
		...
		if (scop_indep == indepsym || s != scop_indep) {
			diag(s->name,  (char*)"can't be declared a parameter by default");
		}
	}
	return s;
}
...
void steppedinstall(Symbol* n, Item* q1, Item* q2, char* units)
{
	...
	previous_str = n->u.str;
	if (seestep) {
		diag((char*)"Only one STEPPED variable can be defined", (char *) 0);
	}
	seestep = 1;
	stepsym = n;
	i = 0;
	...
	while (q1 != q2) {
		...
		i++;
		if (i > 5) {
			diag((char*)"Maximum of 5 steps in a stepped variable",
			     (char *) 0);
		}
	}
	Strcat(buf, "\n");
...
if (!scop) {
	...
	if (using_default_indep) {
		...
		if (indepsym != n) {
			...
			parminstall(indepsym, (char*)"0", (char*)"ms", (char*)"");
		}
		indepsym = (Symbol*)0;
...
#endif
	if (indepsym) {
diag((char*)"Only one independent variable can be defined", (char *) 0);
	}
	indeplist = newlist();
...
void depinstall(int type, Symbol* n, int index, char* from, char* to, char* units, Item* qs, int makeconst, char* abstol)
{
	...
	if (!type && strlen(abstol)>0) {
		printf("abstol = |%s|\n", abstol);
		diag(n->name,  (char*)"tolerance can be specified only for a STATE");
	}
	...
	if (type) {
		...
		c = ';';
		if (qs) {
			diag((char*)"START not legal except in  STATE block", (char *) 0);
		}
	}
	if (index) {
...
void statdefault(Symbol* n, int index, char* units, Item* qs, int makeconst)
{
	...
	if (n->type != NAME && n->type != PRIME) {
		diag(n->name,  (char*)"can't be a STATE");
	}
	if (makeconst) {
		Sprintf(nam, "%s0", n->name);
		s = ifnew_parminstall(nam,  (char*)"0", units, (char*)"");
		if (qs) { /*replace with proper default*/
			parminstall(s, STR(qs), units, (char*)"");
		}
	}
	...
	if ((s = lookup(nam)) == SYM0) {	/* install the prime as a DEP */
		s = install(nam, PRIME);
		depinstall(0, s, index, (char*)"0", (char*)"1", un, ITEM0, 0, (char*)"");
	}
}
		...
		if (q->itemtype == SYMBOL) {
			...
			if ((s->usage & FUNCT) && !(s->subtype & (EXTDEF))) {
				if (q->next->itemtype == SYMBOL && strcmp(SYM(q->next)->name, "(") == 0) {
					...
					if (b == 0) { /* no args */
						vectorize_substitute(q->next,  (char*)"(_p, _ppvar, _thread, _nt");
...
void defarg(Item* q1, Item* q2)			/* copy arg list and define as doubles */
{
	...
	if (q1->next == q2) {
#if VECTORIZE
		vectorize_substitute(insertstr(q2,  (char*)""),  (char*)"_threadargsproto_");
#endif
		return;
	}
	for (q = q1->next; q != q2; q = q->next) {
		if (strcmp(SYM(q)->name, ",") != 0) {
			insertstr(q,  (char*)"double");
		}
	}		
#if VECTORIZE
		vectorize_substitute(insertstr(q1->next,  (char*)""),  (char*)"_threadargsprotocomma_");
#endif
}
...
void lag_stmt(Item* q1, int blocktype)	/* LAG name1 BY name2 */
{
	...
	/* check */
	if (!indepsym) {
		diag((char*)"INDEPENDENT variable must be declared to process",
			 (char*)"the LAG statement");
	}
	if (!(name1->subtype & (DEP | STAT))) {
		diag(name1->name,  (char*)"not a STATE or DEPENDENT variable");
	}
	if (!(name2->subtype & (PARM | nmodlCONST))) {
		diag(name2->name,  (char*)"not a CONSTANT or PARAMETER");
	}
	Sprintf(buf, "lag_%s_%s", name1->name, name2->name);
	if (lookup(buf)) {
		diag(buf,  (char*)"already in use");
	}
	/* create */
...
void queue_stmt(Item* q1, Item* q2)
{
	...
	if (first) {
		first = 0;
		Linsertstr(initfunc,  (char*)"initqueue();\n");
	}
	if (SYM(q1)->type == PUTQ) {
		replacstr(q1,  (char*)"enqueue(");
	}else{
		replacstr(q1,  (char*)"dequeue(");
	}
	
	s = SYM(q2);
	s->usage |= DEP;
	if (!(s->subtype)) {
		diag(s->name,  (char*)"not declared");
	}
	if (s->subtype & ARRAY) {
		Sprintf(buf, "%s, %d);\n", s->name, s->araydim);
	}else{
...
{
	vectorize_substitute(insertstr(q->next,  (char*)"nrn_threads,"),  (char*)"_nt,");
}
...
void table_massage(List* tablist, Item* qtype, Item* qname, List* arglist)
{
	...
	ifnew_parminstall( (char*)"usetable", (char*)"1", (char*)"", (char*)"0 1");
	if (!check_table_statements) {
	...
	if (type == FUNCTION1) {
		if (table) {
diag((char*)"TABLE stmt in FUNCTION cannot have a table name list", (char *)0);
		}
		table = newlist();
		Lappendsym(table, fsym);
	}else{
		if (!table) {
diag((char*)"TABLE stmt in PROCEDURE must have a table name list", (char *)0);
		}
	}
	if (arglist->next == arglist || arglist->next->next != arglist) {
diag((char*)"FUNCTION or PROCEDURE containing a TABLE stmt\n",
 (char*)"must have exactly one argument");
	}else{
	...
	vectorize_substitute(q, buf);
	Lappendstr(procfunc,  (char*)" static int _maktable=1; int _i, _j, _ix = 0;\n");
	Lappendstr(procfunc,  (char*)" double _xi, _tmax;\n");
	ITERATE(q, depend) {
		Sprintf(buf, " static double _sav_%s;\n", SYM(q)->name);
		Lappendstr(procfunc, buf);
	}
	lappendstr(procfunc,  (char*)" if (!usetable) {return;}\n");
	/*allocation*/
	ITERATE(q, table) {
		s = SYM(q);
		if (s->subtype & ARRAY) {
  ...
  _t_%s[_i] = makevector(%d*sizeof(double)); }\n", s->araydim, s->name, ntab+1);
	...
	/* make the table */
	Lappendstr(procfunc,  (char*)" if (_maktable) { double _x, _dx; _maktable=0;\n");
...
#define GLOBFUNC 1
#if !GLOBFUNC
		Lappendstr(procfunc,  (char*)"static int");
#endif
		Lappendstr(procfunc,  (char*)"double");
	}else{
		Lappendstr(procfunc,  (char*)"static int");
	}
...
#endif
	if (type != FUNCTION1) {
		Lappendstr(procfunc,  (char*)"return 0;\n");
	}
	Lappendstr(procfunc,  (char*)"}\n\n"); /* end of new function */

	/* _n_name function for table lookup with no checking */
	if (type == FUNCTION1) {
		Lappendstr(procfunc,  (char*)"static double");
	}else{
		Lappendstr(procfunc,  (char*)"static void");
	}
	...
	vectorize_substitute(procfunc->prev, buf);
#endif
	Lappendstr(procfunc,  (char*)"int _i, _j;\n");
	Lappendstr(procfunc,  (char*)"double _xi, _theta;\n");

	/* usetable */
	Lappendstr(procfunc,  (char*)"if (!usetable) {\n");
	if (type == FUNCTION1) {
		Lappendstr(procfunc,  (char*)"return");
	}
	...
	Lappendstr(procfunc, buf);
	Lappendstr(procfunc,  (char*)"if (isnan(_xi)) {\n");
	if (type == FUNCTION1) {
 		Lappendstr(procfunc,  (char*)" return _xi; }\n");
 	}else{
		ITERATE(q, table) {
			s = SYM(q);
			if (s->subtype & ARRAY) {
...
Sprintf(buf, " %s = _xi;\n", s->name);
	...
	Lappendstr(procfunc,  (char*)"if (_xi <= 0.) {\n");
	if (type == FUNCTION1) {
...
Sprintf(buf, "%s = _t_%s[%d];\n", s->name, s->name, ntab);
	...
	Lappendstr(procfunc,  (char*)"}\n");
	/* table interpolation */
	Lappendstr(procfunc,  (char*)"_i = (int) _xi;\n");
	if (type == FUNCTION1) {
		s = SYM(table->next);
Sprintf(buf, "return _t_%s[_i] + (_xi - (double)_i)*(_t_%s[_i+1] - _t_%s[_i]);\n",
 s->name, s->name, s->name);
		...
		Lappendstr(procfunc,  (char*)"_theta = _xi - (double)_i;\n");
		ITERATE(q, table) {
			s = SYM(q);
			if (s->subtype & ARRAY) {
...
void hocfunchack(Symbol* n, Item* qpar1, Item* qpar2, int hack)
{
   ...
   if (point_process) {
	...
	Lappendstr(procfunc, buf);
	vectorize_substitute(lappendstr(procfunc,  (char*)""),  (char*)"\
  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;\n\
");
	if (point_process) {
		vectorize_substitute(lappendstr(procfunc,  (char*)"  _hoc_setdata(_vptr);\n"),  (char*)"\
...
");
	...
	if (n == last_func_using_table) {
		qp = lappendstr(procfunc,  (char*)"");
		sprintf(buf,"\n#if 1\n _check_%s(_p, _ppvar, _thread, _nt);\n#endif\n", n->name);
		vectorize_substitute(qp, buf);
	}
#endif
	if (n->subtype & FUNCT) {
		Lappendstr(procfunc,  (char*)"_r = ");
	}else{
		Lappendstr(procfunc,  (char*)"_r = 1.;\n");
	}
...
#if VECTORIZE
	if (i) {
		vectorize_substitute(qp,  (char*)"_p, _ppvar, _thread, _nt,");
	}else if (!hack) {
		vectorize_substitute(qp,  (char*)"_p, _ppvar, _thread, _nt");
	}
#endif
...
void vectorize_use_func(Item* qname, Item* qpar1, Item* qexpr, Item* qpar2, int blocktype)
{
	...
	if (SYM(qname)->subtype &  EXTDEF) {
		if (strcmp(SYM(qname)->name, "nrn_pointing") == 0) {
			...
			replacstr(qname,  (char*)"");
			replacstr(qpar1,  (char*)"");
			replacstr(qpar2,  (char*)"");
...
fprintf(stderr, "Notice: Use of state_discontinuity is not thread safe except in a NET_RECEIVE block");
			...
			if (!state_discon_list_) {
				...
				Linsertstr(procfunc,  (char*)"extern int state_discon_flag_;\n");
			}
			lappenditem(state_discon_list_, qpar1->next);
			Insertstr(qpar1->next,  (char*)"-1, &");
		    }
#endif
		}else if (strcmp(SYM(qname)->name, "net_send") == 0) {
			net_send_seen_ = 1;
			if (artificial_cell) {
				replacstr(qname,  (char*)"artcell_net_send");
			}
			Insertstr(qexpr,  (char*)"t + ");
			if (blocktype == NETRECEIVE) {
				Insertstr(qpar1->next,  (char*)"_tqitem, _args, _pnt,");
			}else if (blocktype == INITIAL1){
				Insertstr(qpar1->next,  (char*)"_tqitem, (double*)0, _ppvar[1]._pvoid,");
			}else{
diag((char*)"net_send allowed only in INITIAL and NET_RECEIVE blocks", (char*)0);
			}
		}else if (strcmp(SYM(qname)->name, "net_event") == 0) {
			net_event_seen_ = 1;
			if (blocktype == NETRECEIVE) {
				Insertstr(qpar1->next,  (char*)"_pnt,");
			}else{
diag((char*)"net_event",  (char*)"only allowed in NET_RECEIVE block");
			}
		}else if (strcmp(SYM(qname)->name, "net_move") == 0) {
			if (artificial_cell) {
				replacstr(qname,  (char*)"artcell_net_move");
			}
			if (blocktype == NETRECEIVE) {
				Insertstr(qpar1->next,  (char*)"_tqitem, _pnt,");
			}else{
diag((char*)"net_move",  (char*)"only allowed in NET_RECEIVE block");
			}
		}
		return;
	}
#if 1
	if (qexpr) {
		q = insertstr(qpar1->next,  (char*)"_threadargscomma_");
	}else{
		q = insertstr(qpar1->next,  (char*)"_threadargs_");
	}
#else
	q = insertstr(qpar1->next,  (char*)"");
	if (qexpr) {
		vectorize_substitute(q,  (char*)"_p, _ppvar, _thread, _nt,");
	}else{
		vectorize_substitute(q,  (char*)"_p, _ppvar, _thread, _nt");
	}
#endif
...
void function_table(Symbol* s, Item* qpar1, Item* qpar2, Item* qb1, Item* qb2) /* s ( ... ) { ... } */
{
	...
	if (i == 0) {
		diag((char*)"FUNCTION_TABLE declaration must have one or more arguments:",
			s->name);
	}
...
void watchstmt(Item* par1, Item* dir, Item* par2, Item* flag, int blocktype )
{
	...
	if (blocktype != NETRECEIVE) {
		diag((char*)"\"WATCH\" statement only allowed in NET_RECEIVE block", (char*)0);
	}
	sprintf(buf, "\nstatic double _watch%d_cond(_pnt) Point_process* _pnt; {\n",
		watch_seen_);
	lappendstr(procfunc, buf);
	vectorize_substitute(lappendstr(procfunc,  (char*)""),(char*)"\tdouble* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;\n\t_thread= (Datum*)0; _nt = (_NrnThread*)_pnt->_vnt;\n");
...
Item* protect_astmt(Item* q1, Item* q2) { /* PROTECT, ';' */
	Item* q;
	replacstr(q1,  (char*)"/* PROTECT */_NMODLMUTEXLOCK\n");
	q = insertstr(q2->next,  (char*)"\n _NMODLMUTEXUNLOCK /* end PROTECT */\n");
	protect_include_ = 1;
	return q;
}
...
void nrnmutex(int on, Item* q) { /* MUTEXLOCK or MUTEXUNLOCK */
	...
	if (on == 1) {
		if (toggle != 0) {
			diag((char*)"MUTEXLOCK invoked after MUTEXLOCK", (char*)0);
		}
		toggle = 1;
		replacstr(q,  (char*)"_NMODLMUTEXLOCK\n");
		protect_include_ = 1;
	}else if (on == 0) {
		if (toggle != 1) {
			diag((char*)"MUTEXUNLOCK invoked with no earlier MUTEXLOCK", (char*)0);
		}
		toggle = 0;
		replacstr(q,  (char*)"_NMODLMUTEXUNLOCK\n");
		protect_include_ = 1;
	}else{
		if (toggle != 0) {
			diag((char*)"MUTEXUNLOCK not invoked after MUTEXLOCK", (char*)0);
		}
		toggle = 0;
	}
}

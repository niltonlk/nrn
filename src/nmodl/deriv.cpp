...
void solv_diffeq(Item* qsol, Symbol* fun, Symbol* method, int numeqn, int listnum, int steadystate, int btype)
{
	...
	if (method->subtype & DERF) {
		if (method->u.i == 1) { /* variable step method */
			maxerr_str = (char*)", maxerr";
			IGNORE(ifnew_parminstall((char*)"maxerr", (char*)"1e-5", (char*)"", (char*)""));
		} else {
			maxerr_str = (char*)"";
		}
...
if (deriv_imp_list) {	/* make sure deriv block translation matches method */
	...
	diag((char*)"To use the derivimplicit method the SOLVE statement must precede the DERIVATIVE block\n",
	        (char*)" and all SOLVEs using that block must use the derivimplicit method\n");
	}
...
static Symbol * init_forderiv(Symbol* prime)
{
	...
	if (isdigit(prime->name[1])) { /* higher than 1 */
		if(sscanf(prime->name + 1, "%d%s", &maxindx, name) != 2) {
			diag((char*)"internal error in init_forderiv in init.c", (char *)0);
		}
	}else{
		maxindx = 1;
	...
	if (!forderiv || !(forderiv->subtype & STAT)) {
		diag(name,  (char*)"must be declared as a state variable");
	}
...
nrn_assert(snprintf(units, SB, "%s/%s^%d", base_units, STR(indeplist->prev), indx) < SB);
		depinstall(1, s, forderiv->araydim, (char*)"0", (char*)"1", units, ITEM0, 1, (char*)"");
		s->usage |= DEP;
	}
	if (s->araydim != forderiv->araydim) {
		diag(s->name,  (char*)"must have same dimension as associated state");
	}
...
void massagederiv(Item* q1, Item* q2, Item* q3, Item* q4, int sensused)
{
	...
	replacstr(q1,  (char*)"\nstatic int"); q = insertstr(q3,  (char*)"() {_reset=0;\n");
	derfun = SYM(q2);
	vectorize_substitute(q,  (char*)"(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset=0; int error = 0;\n");

	if (derfun->subtype & DERF && derfun->u.i) {
		diag((char*)"DERIVATIVE merging not implemented", (char *)0);
	}
	...
	derfun->subtype |= DERF;
	if (!deriv_used_list) {
		diag((char*)"No derivative equations in DERIVATIVE block", (char*)0);
	}
...
depinstall(0, s, s->araydim, (char*)"0", (char*)"1", units, ITEM0, 0, (char*)"");
		}
...
diag(state->name,  (char*)": Since higher derivative is being used, this state \
...
if (s->subtype & ARRAY) { int dim = s->araydim;
	...
	if (count == 0) {
		diag((char*)"DERIVATIVE contains no derivatives", (char *)0);
	}
	derfun->used = count;
...
/* don't emit _ode_matsol if the user has defined cvodematsol */
  if (!lookup( (char*)"cvodematsol")) {
	...
	Lappendstr(procfunc,  (char*)"/*END CVODE*/\n");
...
void matchinitial(Item* q1)	/* name */
{
	...
	diag(s->name,  (char*)"must be an initial state parameter");
	return;
}	
...
void matchbound(Item* q1, Item* q2, Item* q3, Item* q4, Item* q5, Symbol* sindex) /* q1name q2'(' q3')' '=' q4exprq5 */
{
	...
	if (!(state->subtype & STAT) && state->type != PRIME) {
		diag(state->name,  (char*)"is not a state");
	}
	if ((state->subtype & ARRAY) && !sindex) {
		diag(state->name,  (char*)"must have an index for the implicit loop");
	}
	if (!(state->subtype & ARRAY) && sindex) {
		diag(state->name,  (char*)"is not an array");
	}
...
void checkmatch(int blocktype) {
	if (blocktype != DERIVATIVE) {
		diag((char*)"MATCH block can only be in DERIVATIVE block", (char *)0);
	}
	matchused = 1; /*communicate with massagederiv*/
	if (match_bound || match_init) {
		diag((char*)"Only one MATCH block allowed", (char *)0);
	}
	if (!indepsym) {
		diag((char*)"INDEPENDENT variable must be declared before MATCH",
		  (char*)"statement");
	}
	match_bound = newlist();
	match_init = newlist();
...
void matchmassage(int nderiv)
{
	...
	*/
	...
	ITERATE(q, match_init) { /* these initializations are done
		...
		if (!(s->subtype & STAT)) {
			diag(s->name,  (char*)"is not a state");
		}
		if (s->subtype & ARRAY) {
			count += s->araydim;
		}else{
	...
	if (nunknown <= 0) {
		diag((char*)"Nothing to match", (char *)0);
	}
...
Sprintf(buf, ";\n _match_value[%d] = ", j);
	...
	Sprintf(buf, "for(_i=0; _i<%d; _i++) { _pmatch_time[_i] = _match_time + _i;\n",
		nunknown);
	Lappendstr(initlist, buf);
	Lappendstr(initlist,  (char*)"_pmatch_value[_i] = _match_value + _i;\n }\n");
	if (count != nderiv) {
...
int cvode_cnexp_success(Item* q1, Item* q2) {
	...
	if ( cvode_cnexp_possible) {
		/* convert Method to nil and the type of the block to
		   PROCEDURE */
		SYM(cvode_cnexp_solve->next)->name = stralloc((char*)"cnexp", 0);
...
sprintf(buf," %s = %s + (1. - exp(dt*(%s)))*(%s - %s)",
		...
		vectorize_substitute(qq->next->next,  (char*)"(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {");

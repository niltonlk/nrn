...
gives us a current equation number */
	...
	replacstr(q1,  (char*)"\nstatic void"); Insertstr(q3,  (char*)"()\n");
	Insertstr(q3->next,  (char*)" int _counte = -1;\n");
	nonfun = SYM(q2);
	if ((nonfun->subtype) & NLINF && nonfun->u.i) {
		diag((char*)"NONLINEAR merging not implemented", (char *)0);
	}
	numlist++;
...
int nonlin_common(Item* q4, int sensused)	/* used by massagenonlin() and mixed_eqns() */
{
	...
	ITERATE(lq, eqnq) {
		...
		if (strcmp(eqtype, "~+") == 0) { /* add equation to previous */
			if (counte == -1) {
				diag((char*)"no previous equation for adding terms", (char *)0);
			}
...
Sprintf(buf, "if(_counte != %d) printf( \"Number of equations, %%d,\
 does not equal number of states, %d\", _counte + 1);\n",
	...
	if (counte == 0) {
		diag((char*)"NONLINEAR contains no equations", (char *)0);
	}
	freeqnqueue();
...
numlist, numlist-1, counts);
	...
	Insertstr(q3,  (char*)"_recurse = 0; if(error) {abort_run(error);}}\n");
	return qret;
}
...
void linterm(Item* q1, Item* q2, int pstate, int sign) /*primary, last ,, */
{
	...
	if (pstate == 1) {
		...
		diag((char*)"more than one state in preceding term", (char *)0);
	}
	Insertstr(q2->next,  (char*)";\n");
}
...
#endif
	if (++nlineq == 0) {
		diag(linblk->name,  (char*)"has no equations");
	}
	Sprintf(buf, "static void %s();\n", SYM(q2)->name);
	Linsertstr(procfunc, buf);
	replacstr(q1,  (char*)"\nstatic void"); Insertstr(q3,  (char*)"()\n");

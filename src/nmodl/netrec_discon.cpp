...
#endif
  if (netrec_cnexp) ITERATE(q, netrec_cnexp) {
    if (SYM(q) == sym) {
      cnexp = STR(q->next);
      break;
    }
...
void netrec_discon() {
  ...
  if (info) {
    ITERATE(q, info) {
      Item* varname = ITM(q); q = q->next;
      Item* equal = ITM(q); q = q->next;
      Item* expr = ITM(q); q = q->next;

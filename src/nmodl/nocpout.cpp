...
*/
char* nmodl_version_ = (char*)"7.7.0";
...
void nrninit() {
	...
	indepinstall(install( (char*)"t", NAME), (char*)"0", (char*)"1", (char*)"100", (Item*)0, (char*)"ms", 0);
...
");
	...
	/*SUPPRESS 765*/\n\
	");
	Lappendstr(defs_list,  (char*)"extern double *getarg();\n");
#if VECTORIZE
    if (vectorize) {
...
//	Lappendstr(defs_list,  (char*)"\n#if defined(__cplusplus)\nextern \"C\" {\n#endif\n"); //TODO - can this be removed? otherwise rely on dirty fix: __cminusminus
	Lappendstr(defs_list,  (char*)"static int hoc_nrnpointerindex = ");
	if (pointercount) {
		q = nrnpointers->next;
		Sprintf(buf, "%d;\n", SYM(q)->used);
	}else{
	...
	if (vectorize) {
		Lappendstr(defs_list,  (char*)"static Datum* _extcall_thread;\n static Prop* _extcall_prop;\n");
	}
#if 0
	Lappendstr(defs_list,  (char*)"/* static variables special to NEURON */\n");
	SYMLISTITER {
...
#endif
	Lappendstr(defs_list,  (char*)"/* external NEURON variables */\n");
	SYMLISTITER {
		s = SYM(q);
	...
	Lappendstr(defs_list,  (char*)"/* declaration of user functions */\n");
	SYMLISTITER {
		s = SYM(q);
	...
	Lappendstr(defs_list,  (char*)"static int _mechtype;\n\
...
"#endif\n\n"
	...
	if (point_process) {
		Lappendstr(defs_list,  (char*)"extern Prop* nrn_point_prop_;\n");
		Lappendstr(defs_list,  (char*)"static int _pointtype;\n");
		Lappendstr(defs_list,  (char*)"static void* _hoc_create_pnt(Object* _ho) { void* create_point_process(int, Object*);\n");
		Lappendstr(defs_list,  (char*)"return create_point_process(_pointtype, _ho);\n}\n");
		Lappendstr(defs_list,  (char*)"static void _hoc_destroy_pnt(void*);\n");
		Lappendstr(defs_list,  (char*)"static double _hoc_loc_pnt(void* _vptr) {double loc_point_process(int, void*);\n");
		Lappendstr(defs_list,  (char*)"return loc_point_process(_pointtype, _vptr);\n}\n");
		Lappendstr(defs_list,  (char*)"static double _hoc_has_loc(void* _vptr) {double has_loc_point(void*);\n");
		Lappendstr(defs_list,  (char*)"return has_loc_point(_vptr);\n}\n");
		Lappendstr(defs_list,  (char*)"static double _hoc_get_loc_pnt(void* _vptr) {\n");
		Lappendstr(defs_list,  (char*)"double get_loc_point_process(void*); return (get_loc_point_process(_vptr));\n}\n");
	}
	/* function to set up _p and _ppvar */
	Lappendstr(defs_list,  (char*)"extern void _nrn_setdata_reg(int, void(*)(Prop*));\n");
	Lappendstr(defs_list,  (char*)"static void _setdata(Prop* _prop) {\n");
#if VECTORIZE
    if (vectorize) {
	Lappendstr(defs_list,  (char*)"_extcall_prop = _prop;\n");
    }else
#endif
    {
	Lappendstr(defs_list,  (char*)"_p = _prop->param; _ppvar = _prop->dparam;\n");
    }
	Lappendstr(defs_list,  (char*)"}\n");
	
	if (point_process) {
		Lappendstr(defs_list,  (char*)"static void _hoc_setdata(void* _vptr) { Prop* _prop;\n");
		Lappendstr(defs_list,  (char*)"_prop = ((Point_process*)_vptr)->_prop;\n");
	}else{
		Lappendstr(defs_list,  (char*)"static void _hoc_setdata() {\n Prop *_prop, *hoc_getdata_range(int);\n");
		Sprintf(buf, "_prop = hoc_getdata_range(_mechtype);\n");
		Lappendstr(defs_list, buf);
	}
	Lappendstr(defs_list,  (char*)"  _setdata(_prop);\n");
	if (point_process) {
		Lappendstr(defs_list,  (char*)"}\n");
	}else{
		Lappendstr(defs_list,  (char*)"hoc_retpushx(1.);\n}\n");
	}
	
	/* functions */
	Lappendstr(defs_list,  (char*)"/* connect user functions to hoc names */\n");
	Lappendstr(defs_list, (char*)"static VoidFunc hoc_intfunc[] = {\n");
   if (point_process) {
	Lappendstr(defs_list, (char*)"0,0\n};\n");
	Lappendstr(defs_list,  (char*)"static Member_func _member_func[] = {\n");
	...
	SYMLISTITER {
		...
		if ((s->subtype & FUNCT)) {
			...
			if (vectorize && !s->no_threadargs) {
				if (s->varnum) {
					Lappendstr(defs_list,  (char*)"_threadargsprotocomma_");
				}else{
					Lappendstr(defs_list,  (char*)"_threadargsproto_");
				}
			}
			for (j=0; j < s->varnum; ++j) {
				Lappendstr(defs_list,  (char*)"double");
				if (j+1 < s->varnum) {
					Lappendstr(defs_list,  (char*)",");
				}
			}
...
sprintf(buf, "static double %s;\n", SYM(q)->name);
	...
	Lappendstr(defs_list,  (char*)"/* declare global and static user variables */\n");
	if (gind) {
		...
		lappendstr(thread_mem_init_list, buf);
		lappendstr(thread_cleanup_list,  (char*)" if (_thread[_gth]._pval == _thread1data) {\n   _thread1data_inuse = 0;\n  }else{\n   free((void*)_thread[_gth]._pval);\n  }\n");
		++thread_data_index;
	}
	gind = 0;
...
s->name, suffix, gind, s->name, gind);
	...
	Lappendstr(defs_list,  (char*)"/* some parameters have upper and lower limits */\n");
	Lappendstr(defs_list,  (char*)"static HocParmLimits _hoc_parm_limits[] = {\n");
	SYMLISTITER {
		s = SYM(q);
		if (s->subtype & PARM) {
			double d1=0., d2=0.;
...
Sprintf(buf, "\"%s\", %g, %g,\n", s->name, d1, d2);
	...
	Lappendstr(defs_list,  (char*)"0,0,0\n};\n");
...
#if VECTORIZE && 0
    if (vectorize) {
diag((char*)"No statics allowed for thread safe models:", s->name);
    }
#endif
			decode_ustr(s, &d1, &d2, buf);
			if (s->subtype & ARRAY) {
	...
	Lappendstr(defs_list,  (char*)"/* connect global user variables to hoc */\n");
	Lappendstr(defs_list, (char*)"static DoubScal hoc_scdoub[] = {\n");
	...
	/* double vectors */
	Lappendstr(defs_list, (char*)"static DoubVec hoc_vdoub[] = {\n");
 	ITERATE(q, syminorder) {
		s = SYM(q);
		if (s->nrntype & NRNGLOBAL && (s->subtype & ARRAY)) {
	...
	Lappendstr(defs_list,  (char*)"0,0,0\n};\n");
	Lappendstr(defs_list,  (char*)"static double _sav_indep;\n");
	if (ba_index_ > 0) {
		Lappendstr(defs_list,  (char*)"static void _ba1()");
	...
	Lappendstr(defs_list,  (char*)"static void nrn_alloc(Prop*);\nstatic void  nrn_init(_NrnThread*, _Memb_list*, int);\nstatic void nrn_state(_NrnThread*, _Memb_list*, int);\n\
");
	if (brkpnt_exists) {
		Lappendstr(defs_list,  (char*)"static void nrn_cur(_NrnThread*, _Memb_list*, int);\nstatic void  nrn_jacob(_NrnThread*, _Memb_list*, int);\n");
	}
	...
	if (point_process) {
		Lappendstr(defs_list,  (char*)"static void _hoc_destroy_pnt(void* _vptr) {\n");
		if (watch_seen_ || for_netcons_) {
			Lappendstr(defs_list,  (char*)"  Prop* _prop = ((Point_process*)_vptr)->_prop;\n");
		}
		if (watch_seen_) {
...
sprintf(buf, "  if (_prop) { _nrn_free_fornetcon(&(_prop->dparam[_fnc_index]._pvoid));}\n");
		...
		Lappendstr(defs_list,  (char*)"  destroy_point_process(_vptr);\n}\n");
	}
	if (cvode_emit) {
	...
	if (destructorfunc->next != destructorfunc) {
		if (! point_process) {
			diag((char*)"DESTRUCTOR only permitted for POINT_PROCESS", (char*)0);
		}
		Lappendstr(defs_list,  (char*)"static void _destructor(Prop*);\n");
	}
	
	if (constructorfunc->next != constructorfunc) {
		Lappendstr(defs_list,  (char*)"static void _constructor(Prop*);\n");
	}
	
	Lappendstr(defs_list,
 (char*)"/* connect range variables in _p that hoc is supposed to know about */\n");
	Lappendstr(defs_list,  (char*)"\
...
");
	...
	if (diamdec) {
		Lappendstr(defs_list,  (char*)"static Symbol* _morphology_sym;\n");
	}
	if (areadec) {
		Lappendstr(defs_list,  (char*)"extern Node* nrn_alloc_node_;\n");
	}
	ITERATE(q, useion) {
		sion = SYM(q);
...
");
	if (point_process) {
		Lappendstr(defs_list,  (char*)" if (nrn_point_prop_) {\n\
...
Sprintf(buf, "	_p = nrn_prop_data_alloc(_mechtype, %d, _prop);\n", parraycount);
	Lappendstr(defs_list, buf);
	Lappendstr(defs_list,  (char*)"	/*initialize range parameters*/\n");
	ITERATE(q, rangeparm) {
		s = SYM(q);
		if (s->subtype & ARRAY) {
			continue;
	...
	Lappendstr(defs_list,  (char*)"\t_prop->param = _p;\n");
...
Sprintf(buf, "	_ppvar = nrn_prop_datum_alloc(_mechtype, %d, _prop);\n", ppvar_cnt);
		...
		Lappendstr(defs_list,  (char*)"\t_prop->dparam = _ppvar;\n");
		Lappendstr(defs_list, (char*)"\t/*connect ionic variables to this model*/\n");
	}
	if (diamdec) {
...
"\t_ppvar[%d]._pval = &prop_ion->param[%d]; /* _ion_di%sdv */\n",
	...
	if (constructorfunc->next != constructorfunc) {
		Lappendstr(defs_list,  (char*)"if (!nrn_point_prop_) {_constructor(_prop);}\n");
		    if (vectorize) {
			Lappendstr(procfunc,  (char*)"\n\
...
");
	...
	Lappendstr(defs_list,  (char*)"static void _initlists();\n");
#if CVODE
	if (cvode_emit) {
		Lappendstr(defs_list,  (char*)" /* some states have an absolute tolerance */\n");
		Lappendstr(defs_list,  (char*)"static Symbol** _atollist;\n");
		Lappendstr(defs_list,  (char*)"static HocStateTolerance _hoc_state_tol[] = {\n");
		ITERATE(q, rangestate) {
			double d1;
			s = SYM(q);
...
Sprintf(buf, "\"%s\", %g,\n", s->name, d1);
	...
	if (net_send_seen_) {
		if (!net_receive_) {
			diag((char*)"can't use net_send if there is no NET_RECEIVE block", (char*)0);
		}
		sprintf(buf, "\n#define _tqitem &(_ppvar[%d]._pvoid)\n", tqitem_index);
		Lappendstr(defs_list, buf);
		if (net_send_delivered_) {
			insertstr(net_send_delivered_,  (char*)"  if (_lflag == 1. ) {*(_tqitem) = 0;}\n");
		}
	}
	if (net_receive_) {
		Lappendstr(defs_list,  (char*)"static void _net_receive(Point_process*, double*, double);\n");
		if (for_netcons_) {
			Lappendstr(defs_list,  (char*)"extern int _nrn_netcon_args(void*, double***);\n");
		}
		if (net_init_q1_) {
			Lappendstr(defs_list,  (char*)"static void _net_init(Point_process*, double*, double);\n");
		}
	}
	if (vectorize && thread_mem_init_list->next != thread_mem_init_list) {
		Lappendstr(defs_list,  (char*)"static void _thread_mem_init(Datum*);\n");
	}
	if (vectorize && thread_cleanup_list->next != thread_cleanup_list) {
		Lappendstr(defs_list,  (char*)"static void _thread_cleanup(Datum*);\n");
	}
	if (uip) {
		lappendstr(defs_list,  (char*)"static void _update_ion_pointer(Datum*);\n");
	}
	if (use_bbcorepointer) {
		lappendstr(defs_list,  (char*)"static void bbcore_write(double*, int*, int*, int*, _threadargsproto_);\n");
		lappendstr(defs_list,  (char*)"extern void hoc_reg_bbcore_write(int, void(*)(double*, int*, int*, int*, _threadargsproto_));\n");
	}
	Lappendstr(defs_list,  (char*)"\
...
");        
	...
	int _vectorized = %d;\n", modbase, vectorize);
	Lappendstr(defs_list, buf);
	q = lappendstr(defs_list,  (char*)"");
	Lappendstr(defs_list,  (char*)"_initlists();\n");
#else
	Sprintf(buf, "void _%s_reg() {\n	_initlists();\n", modbase);
	Lappendstr(defs_list, buf);
#endif
	...
	if (diamdec) {
		Lappendstr(defs_list,  (char*)"\t_morphology_sym = hoc_lookup(\"morphology\");\n");
	}
	ITERATE(q, useion) {
	...
	_pointtype = point_register_mech(_mechanism,\n\
	 ...
	 _hoc_create_pnt, _hoc_destroy_pnt, _member_func);\n",
	   brkpnt_str_, vectorize ? 1 + thread_data_index : 0);
	 	Lappendstr(defs_list, buf);
		if (destructorfunc->next != destructorfunc) {
			Lappendstr(defs_list,  (char*)"	register_destructor(_destructor);\n");
		}
	}else{
		sprintf(buf, "\
	...
	if (vectorize && thread_data_index) {
		...
		if (thread_mem_init_list->next != thread_mem_init_list) {
			Lappendstr(defs_list,  (char*)" _thread_mem_init(_extcall_thread);\n");
			if (gind) {Lappendstr(defs_list,  (char*)" _thread1data_inuse = 0;\n");}
		}
	}
#endif
	Lappendstr(defs_list,  (char*)"_mechtype = nrn_get_mechtype(_mechanism[1]);\n");
	lappendstr(defs_list,  (char*)"    _nrn_setdata_reg(_mechtype, _setdata);\n");
	if (vectorize && thread_mem_init_list->next != thread_mem_init_list) {
		lappendstr(defs_list,  (char*)"    _nrn_thread_reg(_mechtype, 1, _thread_mem_init);\n");
	}
	if (vectorize && thread_cleanup_list->next != thread_cleanup_list) {
		lappendstr(defs_list,  (char*)"    _nrn_thread_reg(_mechtype, 0, _thread_cleanup);\n");
	}
	if (uip) {
		lappendstr(defs_list,  (char*)"    _nrn_thread_reg(_mechtype, 2, _update_ion_pointer);\n");
	}
	if (emit_check_table_thread) {
		lappendstr(defs_list,  (char*)"    _nrn_thread_table_reg(_mechtype, _check_table_thread);\n");
	}
	if (use_bbcorepointer) {
		lappendstr(defs_list,  (char*)"  hoc_reg_bbcore_write(_mechtype, bbcore_write);\n");
	}
	if (nmodl_text) {
		lappendstr(defs_list,  (char*)"#if NMODL_TEXT\n  hoc_reg_nmodl_text(_mechtype, nmodl_file_text);\n  hoc_reg_nmodl_filename(_mechtype, nmodl_filename);\n#endif\n");
	}
	...
	if (i) {
		Lappendstr(defs_list,  (char*)"\tnrn_writes_conc(_mechtype, 0);\n");
	}
	...
	hoc_register_tolerance(_mechtype, _hoc_state_tol, &_atollist);\n");
		if (ion_synonym) {
Lappendstr(defs_list,  (char*)"	hoc_register_synonym(_mechtype, _ode_synonym);\n");
		}
...
located in a section and is not associated with an integrator\n"
	...
	if (net_event_seen_) {
		Lappendstr(defs_list,  (char*)"add_nrn_has_net_event(_mechtype);\n");
	}
	if (net_receive_) {
		Lappendstr(defs_list,  (char*)"pnt_receive[_mechtype] = _net_receive;\n");
		if (net_init_q1_) {
			Lappendstr(defs_list,  (char*)"pnt_receive_init[_mechtype] = _net_init;\n");
		}
...
{\n\
   v = NODEV(_nd);\n\
			...
			insertstr(ITM(q),  (char*)" v = NODEV(_nd);\n");
		}
	...
	if (ldifuslist) {
		Lappendstr(defs_list,  (char*)"\thoc_register_ldifus1(_difusfunc);\n");
		Linsertstr(defs_list,  (char*)"static void _difusfunc(ldifusfunc2_t, _NrnThread*);\n");
	}
    } /* end of not "nothing" */
	Lappendstr(defs_list,  (char*)"\
...
sprintf(buf1, "\tivoc_help(\"help ?1 %s %s/%s\\n\");\n", mechname, buf, finname);
    ...
    if (suffix[0]) {
	Lappendstr(defs_list,  (char*)"hoc_register_limits(_mechtype, _hoc_parm_limits);\n");
	Lappendstr(defs_list,  (char*)"hoc_register_units(_mechtype, _hoc_parm_units);\n");
    }
	Lappendstr(defs_list,  (char*)"}\n"); /* end of _reg */
	if (vectorize && thread_mem_init_list->next != thread_mem_init_list) {
 ...
 *_pdvol = ", n, n);
 ...
 if (_i == %s) {\n  *_pdfcdc = %s;\n }else{ *_pdfcdc=0.;}\n", cfindex, dfdcur);
		...
		lappendstr(procfunc, buf);
		lappendstr(procfunc,  (char*)"  return");
		for (q1 = qdexp; q1 != qb1; q1 = q1->next) {
			lappenditem(procfunc, q1);
		}
		lappendstr(procfunc,  (char*)";\n}\n");
	}
	lappendstr(procfunc,  (char*)"static void _difusfunc(ldifusfunc2_t _f, _NrnThread* _nt) {int _i;\n");
	n = 0;
	ITERATE(q, ldifuslist) {
		s = SYM(q); q = q->next;
		qdexp = ITM(q); q = q->next;
...
void decode_ustr(Symbol* sym, double* pg1, double* pg2, char* s)	/* decode sym->u.str */
{
	...
	case PARM:
		...
		break;
	default:
		diag(sym->name, (char*)" does not have a proper declaration");
	}
	if (s[0] == '0') {s[0] = '\0';}
}
...
void units_reg() {
	...
	Lappendstr(defs_list,  (char*)"static HocParmUnits _hoc_parm_units[] = {\n");
	ITERATE (q, syminorder) {
		s = SYM(q);
		if (s->nrntype & NRNGLOBAL) {
...
void nrn_list(Item* q1, Item* q2)
{
	...
	case NONSPECIFIC:
		...
		break;
	case SECTION:
		diag((char*)"NEURON SECTION variables not implemented", (char *)0);
		break;
	case GLOBAL:
		for (q = q1->next; q != q2->next; q = q->next) {
	...
	case EXTERNAL:
#if VECTORIZE
threadsafe((char*)"Use of EXTERNAL is not thread safe.");
#endif
		for (q = q1->next; q != q2->next; q = q->next) {
		...
		break;
	case POINTER:
threadsafe((char*)"Use of POINTER is not thread safe.");
		plist = &nrnpointers;
		for (q = q1->next; q != q2->next; q = q->next) {
		...
		break;
	case BBCOREPOINTER:
threadsafe((char*)"Use of BBCOREPOINTER is not thread safe.");
		plist = &nrnpointers;
		for (q = q1->next; q != q2->next; q = q->next) {
...
{
	Item* qb, *qv, *q;
	qb = insertstr(q1->prev->prev,  (char*)"/*");
	insertstr(q1,  (char*)"*/\n");
	if (!ba_list_) {
		ba_list_ = newlist();
	}
	...
	insertstr(q1, buf);
	q = q1->next;
	vectorize_substitute(insertstr(q,  (char*)""),  (char*)"double* _p; Datum* _ppvar;");
	qv = insertstr(q,  (char*)"_p = _pp; _ppvar = _ppd;\n");
...
void nrn_use(Item* q1, Item* q2, Item* q3, Item* q4)
{
	...
	if (used) { /* READ gets promoted to WRITE */
		diag((char*)"mergeing of neuron models not supported yet", (char *)0);
...
static Symbol *ifnew_install(char* name)
{
	...
	if ((s = lookup(name)) == SYM0) {
		s = install(name, NAME);
		parminstall(s, (char*)"0", (char*)"", (char*)"");
	}
	return s;
}
...
void nrndeclare() {
	...
	s=lookup( (char*)"diam"); if (s) {
		if (s->nrntype & (NRNRANGE|NRNGLOBAL)) {
diag(s->name,  (char*)"cannot be a RANGE or GLOBAL variable for this mechanism");
		}
		s->nrntype |= NRNNOTP|NRNPRANGEIN; diamdec=1;
	}
	s=lookup( (char*)"area"); if (s) {
		if (s->nrntype & (NRNRANGE|NRNGLOBAL)) {
diag(s->name,  (char*)"cannot be a RANGE or GLOBAL variable for this mechanism");
		}
	...
	s = ifnew_install((char*)"t");
	s->nrntype |= NRNEXTRN | NRNNOTP;
	s = ifnew_install((char*)"dt");
	s->nrntype |= NRNEXTRN | NRNNOTP;
	vectorize_substitute(lappendstr(defs_list,  (char*)"\n#define t nrn_threads->_t\n#define dt nrn_threads->_dt\n"),  (char*)"\n#define t _nt->_t\n#define dt _nt->_dt\n");

	s=lookup( (char*)"usetable"); if (s) { s->nrntype |= NRNGLOBAL | NRNNOTP;}
	s=lookup( (char*)"celsius");if(s){s->nrntype |= NRNEXTRN | NRNNOTP;}
	s=lookup( (char*)"celcius"); if (s) diag((char*)"celcius should be spelled celsius",
					(char *)0);
...
#endif
	if (brkpnt_exists) {
		s = ifnew_install((char*)"_g");
		var_count(s);
	}
	if (debugging_ && net_receive_) {
		s = ifnew_install((char*)"_tsav");
		var_count(s);
	}
}
...
int iondef(int* p_pointercount) {
	...
	if (point_process) {
		ioncount = 2;
		q = lappendstr(defs_list,  (char*)"#define _nd_area  *_ppvar[0]._pval\n");
		q->itemtype = VERBATIM;
	...
	ITERATE(q, useion) {
		...
		if (!uip) {
			uip = newlist();
			lappendstr(uip,  (char*)"extern void nrn_update_ion_pointer(Symbol*, Datum*, int, int);\n");
			lappendstr(uip,  (char*)"static void _update_ion_pointer(Datum* _ppvar) {\n");
		}
		need_style = 0;
		sion = SYM(q);
...
Sprintf(buf, " _ion_di%sdv += (_di%s - %s)/%s",
				...
				Lappendstr(l, buf);
				if (point_process) {
					Lappendstr(l,  (char*)"* 1.e2/ (_nd_area);\n");
				}else{
					Lappendstr(l,  (char*)";\n");
				}
			}
...
void ion_promote(Item* qion)
{
	...
	ITERATE(q, LST(qion->next->next)) { /* promote if WRITE */
		type = iontype(SYM(q)->name, in);
		if (type == IONIN) {
			Lappendstr(defs_list,  (char*)"nrn_check_conc_write(_prop, prop_ion, 1);\n");
			conc = 3;
		}
		if (type == IONOUT) {
			Lappendstr(defs_list,  (char*)"nrn_check_conc_write(_prop, prop_ion, 0);\n");
			conc = 3;
		}
		if (type == IONEREV) {
			rev = 3;
...
void nrn_var_assigned(Symbol* s){
	...
	NRNFIX("celsius");
	if (e) {
diag(s->name,  (char*)"is a special NEURON variable that should not be\n assigned a value\
...
int slist_search(int n, Symbol* s) {
	...
	pi = s->slist_info_;
if (pi == (int*)0) {
	diag(s->name,  (char*)"not really a STATE; Ie. No differential equation for it.\n");
}
	assert(pi);
	for (i=0; i < pi[0]; ++i) {
		if (pi[1+2*i] == n) {
...
printf("|%s||%s||%s|\n",STR(q3), s, buf);
			...
			break;
					...
					if (b == 0) {
diag(SYM(q1)->name,  (char*)"is WRITE but is not a STATE and has no assignment statement");
					}
				}
			}
		}
...
void out_nt_ml_frag(List* p) {
		vectorize_substitute(lappendstr(p,  (char*)"  Datum* _thread;\n"),  (char*)"  double* _p; Datum* _ppvar; Datum* _thread;\n");
		Lappendstr(p,  (char*)"  Node* _nd; double _v; int _iml, _cntml;\n\
...
static int _ode_count(int _type){ return %d;}\n",
   ...
   if (cvode_fun_->subtype == PROCED) {
	cvode_proced_emit();
		...
		Lappendstr(procfunc, buf);
		vectorize_substitute(lappendstr(procfunc,  (char*)"();\n"),  (char*)"(_p, _ppvar, _thread, _nt);\n");
...
the _pp pointer with a pointer to the actual ion model's concentration */
		...
		Lappendstr(procfunc,  (char*)"}\n");
		if (ion_synonym) {
			Lappendstr(defs_list,  (char*)"static void _ode_synonym(int, double**, Datum**);\n");
			Lappendstr(procfunc,  (char*)"\
...
void net_receive(Item* qarg, Item* qp1, Item* qp2, Item* qstmt, Item* qend)
{
	...
	char snew[256];
	if (net_receive_) {
		diag((char*)"Only one NET_RECEIVE block allowed", (char*)0);
	}
	if (!point_process) {
		diag((char*)"NET_RECEIVE can only exist in a POINT_PROCESS", (char*)0);
	}
	net_receive_ = 1;
	deltokens(qp1, qp2);
	insertstr(qstmt,  (char*)"(_pnt, _args, _lflag) Point_process* _pnt; double* _args; double _lflag;");
	i = 0;
	...
	q = insertstr(qstmt,  (char*)"\n{");
	vectorize_substitute(q,  (char*)"\n{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;\n");
	if (watch_seen_) {
		insertstr(qstmt,  (char*)"  int _watch_rm = 0;\n");
	}
	q = insertstr(qstmt,  (char*)"  _p = _pnt->_prop->param; _ppvar = _pnt->_prop->dparam;\n");
	vectorize_substitute(insertstr(q,  (char*)""),  (char*)"  _thread = (Datum*)0; _nt = (_NrnThread*)_pnt->_vnt;");
	if (debugging_) {
	    if (0) {
		insertstr(qstmt,  (char*)" assert(_tsav <= t); _tsav = t;");
	    }else{
		insertstr(qstmt,  (char*)" if (_tsav > t){ extern char* hoc_object_name(); hoc_execerror(hoc_object_name(_pnt->ob), \":Event arrived out of order. Must call ParallelContext.set_maxstep AFTER assigning minimum NetCon.delay\");}\n _tsav = t;");
	    }
	}
	insertstr(qend,  (char*)"}");
	if (!artificial_cell) {
		...
		*/
		Symbol* vsym = lookup( (char*)"v");
		netrec_need_v = 1;
		for (q = qstmt; q != qend; q = q->next) {
			if (q->itemtype == SYMBOL && SYM(q) == vsym) {
				insertstr(qstmt,  (char*)" v = NODEV(_pnt->node);\n");
				insertstr(qend,  (char*)"\n NODEV(_pnt->node) = v;\n");
				netrec_need_v = 0;
				break;
			}
...
diag(s->name,  (char*)":only concentrations can be mentioned in a NET_RECEIVE block");
				}
				/* distinct only */
				for (j=0; j < nion; ++j) {
					if (s == ions[j]) {
				...
				if (j == nion) {
					if (nion >= 10) {
diag((char*)"too many ions mentioned in NET_RECEIVE block (limit 10", (char*)0);
					}
					ions[nion] = s;
					++nion;
				}
...
void net_init(Item* qinit, Item* qp2)
{
	...
	replacstr(qinit,  (char*)"\nstatic void _net_init(Point_process* _pnt, double* _args, double _lflag)");
	sprintf(buf, "    _p = _pnt->_prop->param; _ppvar = _pnt->_prop->dparam;\n");
	vectorize_substitute(insertstr(qinit->next->next, buf),  (char*)"\
...
");
	if (net_init_q1_) {
		diag((char*)"NET_RECEIVE block can contain only one INITIAL block", (char*)0);
	}
	net_init_q1_ = qinit;
	net_init_q2_ = qp2;
}
...
void conductance_hint(int blocktype, Item* q1, Item* q2) {
	...
	if (blocktype != BREAKPOINT) {
		diag((char*)"CONDUCTANCE can only appear in BREAKPOINT block", (char*)0);
	}
	if (!conductance_) {
	...
	if (q2 != q1->next) {
		Symbol* s = SYM(q2);
		if (!ion_declared(s)) {
			diag(s->name,  (char*)"not declared as USEION in NEURON block");
		}

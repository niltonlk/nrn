...
void solv_partial(Item* qsol, Symbol* fun)
{
	...
	if ((dspace = lookup( (char*)"delta_x")) == SYM0) {
		dspace = install( (char*)"delta_x", NAME);
		parminstall(dspace, (char*)"1", (char*)"", (char*)"");
	}
	Sprintf(buf, "%s();\n", fun->name);
	replacstr(qsol, buf);
...
void partial_eqn(Item* q2, Item* q4, Item* q8, Item* q11) /*V' F V G*/
{
	...
	ITERATE(q, parinfo) {
		if (SYM(q) == SYM(q2)) {
diag((char*)"Two partial equations for same state: ", SYM(q8)->name);
		}
	...
	if (!(pv[2]->subtype & STAT)){
		diag(pv[2]->name,  (char*)"is not a STATE");
	}
	if (strcmp(pv[2]->name, pv[0]->name + 1)) {
		diag(pv[2]->name,  (char*)"' must be the time derivative");
	}
	for (i=0; i<4; i++) {
		if (!(pv[i]->subtype & ARRAY) || (pv[i]->araydim != dim)) {
			diag(pv[i]->name,  (char*)"dimension differs from STATE var");
		}
	}
	replacstr(q2->prev,  (char*)"\n/*where partial call goes*/\n");
...
void massagepartial(Item* q1, Item* q2, Item* q3, Item* q6) /*PARTIAL NAME stmtlist '}'*/
{
	...
	if (seepartial++) {
		diag((char*)"Only one partial block currently allowed", (char *)0);
	}
	if (!pv[0]) {
diag(
        (char*)"within the PARTIAL block must occur at least one equation with the syntax ---\n",
        (char*)"~ V' = F*DEL2(V) + G\n");
	}
   ...
   ITERATE(q, parinfo) {
	...
	ITERATE(qb, bndinfo) {
		...
		if (s == pv[2]) {
			if (bnd[qb->itemtype]) {
diag((char*)"Duplicate boundary condition for ", s->name);
			}		
	...
	for (i=0; i<2; i++) {
		if (bnd[i]) { /* Neumann */
			if (bnd[i+2]) { /* and also dirichlet */
				diag((char*)"Neumann and Dirichlet conditions",
				  (char*)"specified on same side");
			}
		} else { /* Neumann not specified */
			if (!bnd[i+2]) { /* neither is dirichlet */
				/* then default Neumann = 0 */
				bnd[i] = newlist();
				Lappendstr(bnd[i],  (char*)"0");
			}
		}
	}
	...
	ITERATE(qb, bndinfo) {
		...
		qb = qb->next;
		if (qb->itemtype != -1) {
			diag((char*)"Boundary condition: No partial equation for ",
			  s->name);
		}
	}
	replacstr(q1,  (char*)"int");
	Insertstr(q3,  (char*)"()\n");
	SYM(q2)->subtype |= PARF;
	SYM(q2)->u.i = 0;	/* not used: but see listnum in solvhandler */
	Insertstr(q6,  (char*)"return 0;\n");

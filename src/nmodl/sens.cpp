...
there is a linearsens call and we must be sure to keep proper state order */
	...
	if (lookup(buf)) {
		diag(buf,  (char*)"is user defined and cant be used for SENS");
	}
	/*this is a time bomb*/
...
depinstall(1, s, SYM(q1)->araydim, (char*)"0", (char*)"1", (char*)"", ITEM0, 0, (char*)"");
			}else{
				depinstall(1, s, 0, (char*)"0", (char*)"1", (char*)"", ITEM0, 0, (char*)"");
			}
			s->usage |= DEP;
if (type == DERIVATIVE) {
			s = lookup(dname);
...
fn, nstate*i);
		...
		IGNORE(ifnew_parminstall(buf,  (char*)"0.05", (char*)"", (char*)""));
		...
		ITERATE(q1, statelist) {
			...
			if (SYM(q1)->subtype & ARRAY) {
depinstall(0, s, SYM(q1)->araydim,(char*) "0", (char*)"1", (char*)"", ITEM0, 0, (char*)"");
			}else{
				depinstall(0, s, 0, (char*)"0", (char*)"1", (char*)"", ITEM0, 0, (char*)"");
			}
			s->usage |= DEP;

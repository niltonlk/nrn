...
diag((char*)"The SOLVE statement must be before the DERIVATIVE block for ", SYM(lq)->name);
		}
	}
#endif
	...
	if (q2) {
		...
		if (strcmp(SYM(q2)->name, "cnexp") == 0) {
			SYM(q2)->name = stralloc((char*)"derivimplicit", SYM(q2)->name);
			add_deriv_imp_list(SYM(q1)->name);
#if CVODE
...
current_line());
 ...
 (char*)"if(error){abort_run(error);}\n");
#endif
	}else{
		replacstr(qtemp,  (char*)"if (error)");
	}
	errstmt = newlist();
	lq = lappendsym(solvq, SYM0);
	LST(lq) = errstmt;
...
fprintf(stderr, "Notice: DISCRETE is not thread safe.\n");
	...
	if (cvodemethod_ == 2) { /*cvode_t*/
				cvode_interface(fun, listnum, 0);
				insertstr(qsol,  (char*)"if (!cvode_active_)");
				cvode_nrn_cur_solve_ = fun;
				linsertstr(procfunc,  (char*)"extern int cvode_active_;\n");
	}
...
fprintf(stderr, "Notice: PARTIAL is not thread safe.\n");
			...
			break;
		default:
			diag((char*)"Illegal or unimplemented SOLVE type: ", fun->name);
			break;
		}
#if CVODE
		if (btype == BREAKPOINT) {
...
#endif
	/* add the error check */
	Insertstr(qsol,  (char*)"error =");
	move(errstmt->next, errstmt->prev, qsol->next);
#if VECTORIZE
	if (errstmt->next == errstmt->prev) {
		vectorize_substitute(qsol->next,  (char*)"");
		vectorize_substitute(qsol->prev,  (char*)"");
	}else{
...
void save_dt(Item* q)	/* save and restore the value of indepvar */
{
	...
	if (first) {
		first = 0;
		Linsertstr(procfunc,  (char*)"double _savlocal;\n");
	}
...
//#endif
			...
			deltaindep = ifnew_parminstall(buf, sval,  (char*)"", (char*)"");
			firstderf = 0;
#if NMODL
#else
Sprintf(buf, "_modl_set_dt(_dt) double _dt; { %s = _dt;}\n",
	...
	if (strcmp(indepsym->name, "t") != 0) {
		diag((char*)"The independent variable name must be `t'", (char *)0);
	}
#else
...
if (type == DERF) {
	...
	_y = _break - %s; _dt = %s;\n", indepsym->name, "dt");
 	Insertstr(qsol, buf);
 	Insertstr(qsol,  (char*)"_nstep = (int)(_y/_dt + .9);\n if (_nstep==0) _nstep = 1;\n");
...
#if VECTORIZE
		vectorize_substitute(initfunc->prev,  (char*)"");
#endif
	}
	called++;
}
...
diag((char*)"STEADYSTATE requires all SOLVE's of this DERIVATIVE block to use the\n\
...
diag((char*)"STEADYSTATE requires all SOLVE's of this KINETIC block to use the\n\
same method (`advance' or `sparse'). :", fun->name);
				}
				break;
			default:
diag((char*)"STEADYSTATE only valid for SOLVEing a KINETIC or DERIVATIVE block:", fun->name);
				break;
			}
		}
		q = q->next->next;

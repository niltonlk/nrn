...
void consistency()
{
	...
	SYMITER(NAME) {
		...
		tu = s->usage;
		if ((tu & DEP) && (tu & FUNCT))
			diag(s->name, (char*)" used as both variable and function");
		if ((t == 0) && tu)
			Fprintf(stderr,
				"Warning: %s undefined. (declared within VERBATIM?)\n", s->name);
	}
	if (err) {
		diag((char*)"multiple uses for same variable", (char *) 0);
	}
	if (indepsym == SYM0) {
		diag((char*)"Independent variable is not defined", (char *)0);
	}
...
diag((char*)" cannot be declared as a STATE\n", (char*)0);
			}
		}
	}
}

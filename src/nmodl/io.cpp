...
{				/* copy input verbatim to intoken up to END*s
	...
	for (;;) {
		...
		if (cp == (char *) 0) {
			linenum = l;
			diag(s,  (char*)"block goes to end of file");
		}
		if (isend(s, buf)) {
			break;
		}
...
char* Fgets(char* buf, int size, FILE* f) {
	...
	for(i=0; i < size; ++ i) {
		...
		if (c < 0 || c > 127) {
			...
			*p='\0';
			if (!in_comment_) {
				diag((char*)"Non-Ascii character in file:", buf);
			}
...
int unGetc(int c)
{
	...
	if (ctp > inlinebuf[whichbuf]) {
		...
		*ctp = c;
		...
		diag((char*)"internal error in unGetc",  (char*)"");
	}
	return c;
}
...
char *Gets(char* buf)
{
	...
	if (c == '\n') {
		...
		*cp++ = '\0';
		...
		diag((char*)"internal error in Gets()",  (char*)"");
	}
	return (char *) 0;
}
				 ...
				 * keyword */
{
	if (ctp != '\0') {	/* can only be called after successful Gets */
		...
		diag((char*)"internal error in unGets()",  (char*)"");
	}
}
#endif

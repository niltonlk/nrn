...
void disc_var_seen(Item* q1, Item* q2, Item* q3, int array)	/*NAME '@' NUMBER --- array flag*/
{
	...
	s = SYM(q1);
	if (num < 1) {
		diag((char*)"Discrete variable must have @index >= 1", (char *)0);
	}  
	num--;
	if (!(s->subtype & STAT)) {
	diag(s->name,  (char*)"must be a STATE for use as discrete variable");
	}
	if (array && !(s->subtype & ARRAY)) {
		diag(s->name,  (char*)"must be a scalar discrete variable");
	}
	if (!array && (s->subtype & ARRAY)) {
		diag(s->name,  (char*)"must be an array discrete variable");
	}
	if (s->discdim <= num) {
		s->discdim = num+1;
	}

...
static char	*dfile	= (char*)NEURON_DATA_DIR"/lib/nrnunits.lib";
#else
static char	*dfile	= (char*)"/usr/lib/units";
#endif
#endif
...
#else
static char	*dfilealt	= (char*)"../../share/lib/nrnunits.lib";
#endif
#endif
...
void install_units(char* s1, char* s2) /* define s1 as s2 */
{
	...
	if (tp->name) {
		...
		diag((char*)"Units redefinition", (char *)0);
	}
	tp->name = s1;
...
if (unitonflag && up->dim[0] != 9 && usp->dim[0] != 9) {
	for (i=0; i<NDIM; i++) {
		if (up->dim[i] != usp->dim[i]) {
			...
			fprintf(stderr, "\nunits:");
			units(up);
diag((char*)"The units of the previous two expressions are not conformable",(char*)"\n");
		}
	}
	if (OUTTOLERANCE(up->factor, usp->factor)) {
		chkfperror();
...
{
	if (fperrc) {
		diag((char*)"underflow or overflow in units calculation", (char *)0);
	}
}
...
void dimensionless() {
	...
	for (i=0; i<NDIM; i++) {
		if (usp->dim[i] != 0) {
			units(usp);
diag((char*)"The previous expression is not dimensionless", (char *)0);
		}
	}
}
...
diag((char*)"Bad MODLUNIT environment variable. Cant open:", s);
		}
	}
...
loop:
	...
	(%s)	(units)\n", name);
	diag((char*)"Cannot recognize the units: ", name);

...
void nrn_unit_chk() {
	...
	unit_chk((char*)"v",        (char*)"millivolt");
	unit_chk((char*)"t",        (char*)"ms");
	unit_chk((char*)"dt",       (char*)"ms");
	unit_chk((char*)"celsius",  (char*)"degC");
	unit_chk((char*)"diam",     (char*)"micron");
	unit_chk((char*)"area",     (char*)"micron2");

    if (breakpoint_local_seen_ == 0 || conductance_seen_ == 0) {
	ITERATE(q, current) {
		if (point_process) {
			unit_chk(SYM(q)->name, (char*)"nanoamp");
		}else{
			unit_chk(SYM(q)->name, (char*)"milliamp/cm2");
		}
	}
    }
	ITERATE(q, concen) {
		unit_chk(SYM(q)->name, (char*)"milli/liter");
	}
	ITERATE(q, potential) {
		unit_chk(SYM(q)->name, (char*)"millivolt");
	}
}

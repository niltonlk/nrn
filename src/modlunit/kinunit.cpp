...
Fprintf(stderr, "but the quantity units of the first term is: %s\n", unit_str());
			diag((char*)"Inconsistent material quantity units\n",
			  (char*)"Need a correct COMPARTMENT statement");
		}
	}
	...
	if (type->itemsubtype == REACT1 || type->itemsubtype == '-') {
		...
		unit_div();
		if (unit_diff()) {
			react_unit_err((char*)"forward", &uflux);
		}
		unit_pop();
	}
	...
	if (type->itemsubtype == REACT1) {
		...
		unit_div();
		if (unit_diff()) {
			react_unit_err((char*)"backward", &uflux);
		}
		unit_pop();
	}
...
diag((char*)"Inconsistent flux units", (char *)0);
		}
		unit_pop();
	}
...
static void set_flux_units(unit* up)
{
	...
	if ((s = lookup((char*) "f_flux")) == SYM0) {
		s = install((char*) "f_flux", NAME);
	}
	s->u.str = stralloc(buf, (char *)0);
	if ((s = lookup((char*) "b_flux")) == SYM0) {
		s = install((char*) "b_flux", NAME);
	}
	s->u.str = stralloc(buf, (char *)0);
...
Fprintf(stderr, "But the users %s rate units are: %s\n", s, Unit_str(up));
	diag((char*)"inconsistent reaction units", (char *)0);
}
...
void unit_compartlist(Item* q)
{
	...
	if (ustr) {
		diag(SYM(q)->name,  (char*)"already in previous COMPARTMENT");
	}
...
void unit_ldifuslist(Item* q, int flag)
{
	...
	ustr = (char *)(ITMA(SYM(q)->info)[6]);
	if (!ustr) {
		diag(SYM(q)->name,  (char*)"not declared in previous COMPARTMENT");
	}
	Unit_push((char*) "micron4/ms");
	if (!unit_cmp_exact()) {
		unit_pop();
diag(unit_str(),  (char*)": relevant area * diffusion constant must\n   be micron2 micron2/ms (1-21 m4/s)");
	}
	unit_pop();
	Unit_push((char*) "micron2");
	Unit_push(ustr);
	if (!unit_cmp_exact()) {
		diag(ustr,  (char*)": With LONGDITUDINAL_DIFFUSION the compartment \

...
void declare(long subtype, Item* q, Item* qa)
{
	...
	if (!sym->subtype) { /* not previously declared */
		...
		if (subtype&(modlunitCONST|DEP|STAT) && sym->subtype&(modlunitCONST|DEP|STAT)){
			...
			diag((char*)"Multiple inconsistent declarations of ", sym->name);
		}
	}
	declare_array(sym);
...
{
	/*ARGSUSED*/
	diag((char*)"promotion not programmed yet", (char *)0);
	return 0;
}
...
diag((char*)"Only one SWEEP declaration is allowed", (char *)0);
		}
		scop_indep = qa;
	}
}
...
void define_value(Item* q1, Item* q2)
{
	...
	s = SYM(q1);
	if (s->subtype) {
		diag(s->name,  (char*)"already declared");
	}
	if (s->usage) {
		diag(s->name,  (char*)"used before DEFINE'ed");
	}
	s->type = DEFINEDVAR;
...
void decdim(Symbol* s, Item* q)
{
	...
	if (s->araydim < 1) {
		diag(s->name,  (char*)"Array index must be > 0");
	}
}
...
listtype(Item* q)
{
	...
	if (i > 1) {
		diag((char*)"internal error inlisttype: First element of LIST is a LIST", (char *)0);
	}
	switch (q->itemtype) {
		case SYMBOL:
			break;
		...
		case ITEM:
			...
			break;
		default:
			diag((char*)"internal error in listtype: SYMBOL not first element", (char *)0);
	}
	return q;
}
...
void declare_implied()
{
	...
	if (!indepsym) {
		...
		save = intoken;
		intoken = newlist();
		name = putintoken((char*) "t", NAME, 0);
		units = putintoken((char*) "ms", STRING, UNITS);
		from = putintoken((char*) "0", INTEGER, INTEGER);
		to = putintoken((char*) "1", INTEGER, INTEGER);
		with = putintoken((char*) "WITH", NAME, 0);
		num = putintoken((char*) "1", INTEGER, INTEGER);
	...
	if (!indepsym) {
		diag((char*)"No INDEPENDENT variable has been declared", (char*)0);
	}
	SYMITERALL {
		if (!s->subtype) {
			sbase = basestate(s);
			if (s->type == PRIME) {
				if (!sbase) {
diag(s->name,  (char*)"is used but its corresponding STATE is not declared");
				}
				s->subtype = DEP;
				if (nprime(s->name) == 1) {

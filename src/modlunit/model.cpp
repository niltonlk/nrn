...
static void openfiles(int argc, char* argv[])
{
	...
	if (argc == 2) {
		...
		diag((char*)"Usage:",  (char*)"modl prefixto.mod [inputfile]");
	}
	if ((fin = fopen(finname, "r")) == (FILE *) 0) {
		Sprintf(finname, "%s.mod", modprefix);
		if ((fin = fopen(finname, "r")) == (FILE *) 0) {
			diag((char*)"Can't open input file: ", finname);
		}
	}
}
...
void printitem(Item* q, FILE* fp)
{
		...
		default:
			Fprintf(stderr, "\nq->itemtype = %d\n", q->itemtype);
			diag((char*)"printlist handles only a few types of items",
				(char *)0);
			break;
		}
	fflush(fp);

...
static int in_place_data_realloc() {
	...
	for (int i=0; i < n_memb_func; ++i) if (types[i]) {
		...
		FOR_THREADS(nt) {
			for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) if (i == tml->index) {
		...
		FOR_THREADS(nt) {
			for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) if (i == tml->index) {
				...
				if (s) for (int j=0; j < ml->nodecount; ++j) {
					(*s)(ml->pdata[j]);
				}
			}
		}
...
//			printf("%d extra %s, looked at %d, found %d\n", extra, memb_func[i].sym->name, looked_at, found);
	...
	FOR_THREADS(nt) {
		// make a map
		for (int i=0; i < n_memb_func; ++i) { mlmap[i] = 0; }
		for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) {
...
//		printf("end nrn_prop_is_cache_efficient %d\n", nrn_prop_is_cache_efficient());
	...
	i=0; for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) { ++i; }
	fprintf(f, "%d\n", i);
	for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) {
		Memb_list* ml = tml->ml;
		i = tml->index;
		...
		if (datumpools_[i]) {sz2 = datumpools_[i]->d2(); }
fprintf(f, "%d %d %d %d %d %s\n", i, sz1, sz2, ntget, cnt, memb_func[i].sym->name);
    ...
    for (it = 0; it < nrn_nthread; ++it) {
	NrnThread* nt = nrn_threads + it;
	for (i=0; i < n_memb_func; ++i) { mlmap[i] = 0; } //unnecessary but ...
	// how many prop used in this thread
	int cnt=0; for (NrnThreadMembList* tml = nt->tml; tml; tml = tml->next) {

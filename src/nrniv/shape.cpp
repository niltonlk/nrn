...
inline float norm(float x, float y) { return (x*x + y*y);}
...
IFGUI
	...
	if (!ifarg(1)) {
		...
		s->rotate(*getarg(1), *getarg(2), *getarg(3),
			*getarg(4), *getarg(5), *getarg(6)
		);
	}
ENDGUI
#endif
...
IFGUI
	...
	if (hoc_is_object_arg(1)) {
		...
		s->point_mark(*hoc_objgetarg(1), colors->color(int(*getarg(2))),
			 style, size);
	}else{
...
ShapeView::ShapeView(ShapeScene* s) : View(
...
ShapeView::ShapeView(ShapeScene* s, Coord* x) : View(
	x[0],x[1],x[2],x[3],
	s, x[6], x[7]
) {
	Coord x1, y1, x2, y2;
...
ShapeScene::ShapeScene(SectionList* sl) : Graph(0)
{
	nrn_define_shape();
	...
	picker()->add_menu("Redraw Shape",
		new ActionCallback(ShapeScene)(this, &ShapeScene::flush));
...
void ShapeScene::rotate(Coord xorg, Coord yorg, Coord zorg,
...
ColorValue* ShapeScene::color_value() { return color_value_;}
PolyGlyph* ShapeScene::shape_section_list() { return sg_;}
...
void ShapeScene::view(Rubberband* rb) {
	...
	ViewWindow* w = new ViewWindow(
	    v = new View((x2+x1)/2, (y1+y2)/2, x2 - x1, this,
	      r - l, (r - l)*(y2 - y1)/(x2 - x1)),
	    "Shape"
	);
	const Event& e = rb->event();
	w->place(l + e.pointer_root_x() - e.pointer_x(),
	    b + e.pointer_root_y() - e.pointer_y());
	w->map();
}
...
#if FASTIDIOUS
void ShapeSection::fastidious_draw(Canvas* c, const Color* color, int i1, float a1, float a2) const {
	int i;
...
void ShapeSection::bevel_join(Canvas* c, const Color* color, int i, float d) const {
	...
	if (b && (perp1[0] != perp2[0] || perp1[1] != perp2[1])) {
		Coord xt[4], yt[4];
		xt[0] = x + d*perp1[0]; yt[0] = y + d*perp1[1];		
		xt[1] = x - d*perp2[0]; yt[1] = y - d*perp2[1];		
		xt[2] = x - d*perp1[0]; yt[2] = y - d*perp1[1];		
		xt[3] = x + d*perp2[0]; yt[3] = y + d*perp2[1];		
		int i;
		c->new_path();
		c->move_to(xt[0], yt[0]);
		for (i = 1; i < 4; ++i) {
...
void ShapeSection::trapezoid(Canvas* c, const Color* color, int i) const{
	trapezoid(c, color, x_[i-1], y_[i-1], x_[i], y_[i],
		Math::abs(sec_->pt3d[i-1].d)/2.,
		Math::abs(sec_->pt3d[i].d)/2.
	);
}

void ShapeSection::trapezoid(Canvas* c, const Color* color,
	float x1, float y1, float x2, float y2, float d1, float d2) const{
	...
	if (OcIdraw::idraw_stream) {
		Coord xt[4], yt[4];
		d = d1;
		xt[0] = x1 + rx*d; yt[0] = y1 + ry*d;
		xt[1] = x1 - rx*d; yt[1] = y1 - ry*d;
		d = d2;
		xt[2] = x2 - rx*d; yt[2] = y2 - ry*d;
		xt[3] = x2 + rx*d; yt[3] = y2 + ry*d;
		OcIdraw::polygon(c, 4, xt, yt, color, 0, true);
	}
}
...
void ShapeSection::pick(Canvas*, const Allocation&, int depth, Hit& h) {
	if (!good() || ! h.event() || h.event()->type() != Event::down) return;
	Coord x = h.left();
	Coord y = h.bottom();
	if (! near_section(x, y, XYView::current_pick_view()->x_pick_epsilon())) return;
...
bool ShapeSection::near_section(Coord x, Coord y, Coord mineps)const {
		...
		for (int i=1; i < n; ++i) {
			if (MyMath::near_line_segment(
			  ...
			  x_[i-1], y_[i-1],
			  x_[i], y_[i],
			  Math::max(float(Math::abs(sec_->pt3d[i-1].d)/2.), float(mineps))
			)) {
				return true;
			}	
		}
...
float ShapeSection::how_near(Coord x, Coord y)const {
	...
	float d2, d = 1e20;
	for (int i=1; i < n; ++i) {
		d2 = MyMath::distance_to_line_segment(
			  x, y,
			  x_[i-1], y_[i-1],
			  x_[i], y_[i]
		);
...
int ShapeSection::get_coord(double a, Coord& x, Coord& y)const {
	...
	if (i == n) {
		...
		double frac = (arc - sec_->pt3d[i-1].arc)
			/ (sec_->pt3d[i].arc - sec_->pt3d[i-1].arc);

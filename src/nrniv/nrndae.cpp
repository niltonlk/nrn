...
//hoc_execerror(secname(nodes_[i]->sec), "NrnDAE: Referring to an extracellular layer but\nextracellular is not inserted.");
...
NrnDAE::NrnDAE(Matrix* cmat, Vect* const yvec, Vect* const y0, int nnode,
                   Node** const nodes, Vect* const elayer, void (*f_init)(void* data), void* const data)
        : y_(*yvec), yptmp_((Object*) NULL), cyp_((Object*) NULL), f_init_(f_init), data_(data) {
//printf("NrnDAE %lx\n", (long)this);
    if (cmat) {

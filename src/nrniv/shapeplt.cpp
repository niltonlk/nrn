...
ShapePlot::ShapePlot(Symbol* sym, SectionList* sl) : ShapeScene(sl) {
	...
	picker()->add_menu("Plot What?", new ActionCallback(ShapePlotImpl)(
		spi_, &ShapePlotImpl::select_variable));
	picker()->add_menu("Variable scale", new ActionCallback(ShapePlotImpl)(
		spi_, &ShapePlotImpl::scale));
	picker()->add_radio_menu("Time Plot", new ActionCallback(ShapePlotImpl)(
		spi_, &ShapePlotImpl::time));
	picker()->add_radio_menu("Space Plot", new ActionCallback(ShapePlotImpl)(
		spi_, &ShapePlotImpl::space));
	picker()->add_radio_menu("Shape Plot", new ActionCallback(ShapePlotImpl)(
		spi_, &ShapePlotImpl::shape));
...
void ShapePlotImpl::scale() {
	...
	y = sp_->color_value()->high();	
        if (var_pair_chooser("Variable range scale", x, y,
...
//printf("graphid_=%g hoc_ac_=%g\n", graphid_, hoc_ac_);
		...
		if (spi_->graphid_ == hoc_ac_) {
			...
			oc.run(buf);
			ss->setColor(colors->color(spi_->colorid_),
				ShapeScene::current_pick_scene());
			++spi_->colorid_;
		}else{
			spi_->graphid_ = 0;
		}
...
//printf("ColorValue %s\n", name.string());
			...
			printf("Cannot open %s: Using built-in colormap for shapeplot\n", name.string());
				...
				if (csize) {
					...
					csize = 0;
					while(fscanf(f, "%d %d %d", &r, &g, &b) == 3) {
						crange[csize] = new Color(
						  ColorIntensity(r/256.),
		...
		if (csize == 0) {
			...
			for ( csize=0; spec[csize*3] != -1; csize++) {
				crange[csize] = new Color(
				  ColorIntensity(spec[csize*3]/256.),
...
const Color* ColorValue::get_color(float val) const {
    ...
    if (csize_) {
	if (x > .99)return crange_[csize_-1];
	else if (x < 0) return crange_[0];
	else return crange_[int(csize_ * x)];
    }else{
	if (x > .99)return crange[csize-1];
	else if (x < 0) return crange[0];
	else return crange[int(csize * x)];
    }
}

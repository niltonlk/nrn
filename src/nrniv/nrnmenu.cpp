...
IFGUI
   ...
   if (!ifarg(1)) {
	...
	for (sp = hoc_built_in_symlist->first; sp; sp = sp->next) {
		if (sp->type == MECHANISM && sp->subtype != MORPHOLOGY
		  && has_globals(sp->name)) {
...
//printf("returned %s\n", sname.string());
	...
	if (type == nrnocCONST) {
		if (x1 < 0) {
			...
			p = sec->prop;
			if (p->dparam[4].val != 1) {
				hoc_ivpvaluerun("Rall",&sec->prop->dparam[4].val, "diam_changed = 1", 1, 0, hoc_var_extra("rallbranch"));
			}
		}
	}else{
		if (x1 < 0) {
...
hoc_ivpvaluerun(vsym->name, hoc_val_pointer(buf), buf2, 1, 0, vsym->extra);
					 }else{

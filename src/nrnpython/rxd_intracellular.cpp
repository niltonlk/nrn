 ...
 */
extern "C" void set_hybrid_data(int64_t* num_1d_indices_per_grid, int64_t* num_3d_indices_per_grid, int64_t* hybrid_indices1d, int64_t* hybrid_indices3d, int64_t* num_3d_indices_per_1d_seg, int64_t* hybrid_grid_ids, double* rates, double* volumes1d, double* volumes3d, double* dxs)
{
    Grid_node* grid;
    int i, j, k, id;
    int grid_id_check = 0;
    ...
    for(int i = line_start; i < line_stop - 1; i += 2)
    {
        ...
        for(int j = 0; j < N; j++)
        {
            ...
            RHS[j] = CVodeRHS[current_index] 
                         - dt * (delta_x[current_index]/SQ(dx) 
                                + delta_y[current_index]/SQ(dy)
        ...
        prev = alphas[current_index] * dcs[current_index] / (alphas[current_index] + alphas[next_index]);
        diag[N-1] = 1.0 + dt * prev / SQ(dx);
        l_diag[N-2] = -dt * prev / SQ(dx);
...
static void variable_step_y(ICS_Grid_node* g, int line_start, int line_stop, int node_start, double* states, double* RHS, double* scratchpad, double* alphas, double *dcs, double dt)
{
    ...
    for(int i = line_start; i < line_stop - 1; i += 2)
    {
        ...
        {
            current_index = ordered_y_nodes[ordered_index];
            RHS[j] = states[current_index] - dt * delta[current_index]
                         / (alphas[current_index] * SQ(dy));
            ordered_index++;
        }
...
void ics_ode_solve_helper(ICS_Grid_node* g, double dt, double* CVodeRHS)
{
    ...
    if(g->ics_adi_dir_x->dcgrid == NULL)
    {
        ...
        ics_find_deltas(x_line_start, x_line_stop, x_node_start, delta_x,
                        x_line_defs, x_ordered_nodes, CVode_states_copy,
                        g->ics_adi_dir_x->dc, alphas);
